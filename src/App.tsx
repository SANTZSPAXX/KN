...
import PortariaPage from "./components/PortariaPage";
...
import FiftyMegaTools from "./components/FiftyMegaTools";
import PageSeoManager from "./components/PageSeoManager";
import ProjectSimulatorView from "./components/ProjectSimulatorView";
...
export default function App() {
  ...
  const [openBreadcrumbMenu, setOpenBreadcrumbMenu] = useState<string | null>(null);
  ...
  const [ferramentasSubTab, setFerramentasSubTab] = useState<"fiscais" | "sandbox" | "fifty" | "gqcode">("fiscais");
  ...
  useEffect(() => {
    const handleUrlChange = () => {
      ...
      if (segments.length === 0) {
        ...
        if (firstSegment === "gqcode") {
          setActiveTab("ferramentas");
          setFerramentasSubTab("gqcode");
          setSelectedPost(null);
          setSelectedProduct(null);
          setSelectedTool(null);
          setSelectedApp(null);
        ...
        if (firstSegment === "gadgets") {
          setActiveTab("ferramentas");
          setFerramentasSubTab("sandbox");
          setSelectedPost(null);
          setSelectedProduct(null);
          setSelectedTool(null);
          setSelectedApp(null);
        ...
        const validTabs = ["inicio", "produtos", "ferramentas", "apps", "promocoes", "apis", "status", "blog", "admin-dashboard", "sobre", "cursos", "playground", "diagnostico", "portaria", "logistica", "simulador"];
  ...
  const [isGlobalDropdownOpen, setIsGlobalDropdownOpen] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    if (selectedProduct) {
      ...
      const tabTitleMap: Record<string, string> = {
        ...
        portaria: "Portaria Inteligente & Controle de Visitantes e Pátio | KoreNexus",
        logistica: "WMS Inteligente, Emissão de NF-e & Gestão de Estoque e CRM | KoreNexus",
        simulador: "Simulador de Projetos & Orçamento de Sistemas | KoreNexus Jundiaí",
        "admin-dashboard": "SSO KN | Gestão de Dados ERP | KoreNexus",
      };
      ...
      const tabDescMap: Record<string, string> = {
        ...
        portaria: "Sistema completo de controle de acesso de visitantes, cadastro bio-facial, registro OCR de placas de veículos e gestão integrada de chaves no barramento ERP da KoreNexus para empresas industriais e comerciais.",
        logistica: "Controle de estoque síncrono, movimentações de armazenamento (WMS), emissão de notas fiscais de saída, CRM de vendas de hardware e diagnóstico avançado com ferramentas estatísticas de ressuprimento.",
        simulador: "Simule e estime os custos, infraestrutura, APIs, inteligência artificial e prazos para o desenvolvimento do seu aplicativo, site ou ERP corporativo personalizado em Jundiaí e região.",
        "admin-dashboard": "Gerencie as tabelas de dados de produtos, ferramentas, planilhas inteligentes, posts do blog e chaves de configurações gerais do sistema ERP no painel seguro SSO KN.",
      };
    ...
    try {
      ...
      try {
        ...
        if (res.ok) {
          ...
          // Ensure we got a valid SpreadsheetData format and not a corrupt or HTML redirect response
          if (parsed && Array.isArray(parsed.produtos) && Array.isArray(parsed.ferramentas)) {
            localData = parsed;
            setDb(localData);
            setLoading(false); // Disable main loading spinner instantly!
          } else {
            console.warn("API de planilha retornou dados corrompidos ou em formato inválido, ignorando carregamento otimista.");
          }
        }
      } catch (localErr) {
  ...
  useEffect(() => {
    ...
    eventSource.onerror = (err) => {
      liveSseErrors++;
      if (liveSseErrors >= 3) {
        console.warn("[SSE] Múltiplos erros na inscrição de notificações ao vivo. Fechando conexão síncrônica silenciosamente para poupar banda.");
        eventSource.close();
      }
    };
  ...
  const renderBreadcrumbs = () => {
    ...
    const wrapWithTooltip = (element: React.ReactNode, pathDescription: string, key: string, isActive: boolean = false) => {
      const content = isActive ? (
        <div className="relative px-2.5 py-1 rounded-lg bg-cyan-500/10 border border-cyan-500/35 text-white font-semibold shadow-[0_0_15px_rgba(6,182,212,0.18)] flex items-center gap-1.5 transition-all duration-300">
          {element}
          {/* Pulsing cyan gradient (shimmer) along the bottom border to guide user navigation & reinforce technology theme */}
          <span className="absolute bottom-[-1px] left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-400 via-cyan-300 to-transparent shadow-[0_1px_6px_rgba(34,211,238,0.8)] animate-pulse" />
        </div>
      ) : (
        element
      );
    ...
    // Shared Helper to render breadcrumb with active states, pop-up dropdown list & shimmer pulsing lines
    const renderBreadcrumbWithDropdown = (
      key: string,
      ...
      const isMenuOpen = openBreadcrumbMenu === key;

      const triggerElement = (
        <div className="flex items-center gap-1 min-w-0 relative">
          <div className={`${isActive ? "relative px-2.5 py-1 rounded-lg bg-cyan-500/10 border border-cyan-500/35 text-white font-semibold shadow-[0_0_15px_rgba(6,182,212,0.18)]" : ""} flex items-center gap-1.5 min-w-0 transition-all duration-300`}>
            {labelElement}
            {isActive && (
              /* Pulsing cyan gradient (shimmer) along the bottom border to guide user navigation & reinforce technology theme */
              <span className="absolute bottom-[-1px] left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-cyan-400 via-cyan-300 to-transparent shadow-[0_1px_6px_rgba(34,211,238,0.8)] animate-pulse" />
          ...
          {options && options.length > 0 && (
            <button
              onClick={(e) => {
                e.preventDefault();
                e.stopPropagation();
                setOpenBreadcrumbMenu(isMenuOpen ? null : key);
              }}
              className={`p-1 rounded-md hover:bg-slate-800/85 hover:text-cyan-400 transition cursor-pointer flex items-center justify-center shrink-0 ${isMenuOpen ? "text-cyan-400 bg-slate-800/90" : "text-slate-500 hover:text-slate-350"}`}
              title="Alternar rápida de página"
            >
              <ChevronDown className={`h-2.5 w-2.5 transition-transform duration-200 ${isMenuOpen ? "rotate-180" : ""}`} />
            </button>
          )}

          {/* Local Pop-up Menu */}
          {isMenuOpen && options && options.length > 0 && (
            <>
              ...
              <div 
                ...
                onClick={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
                  setOpenBreadcrumbMenu(null);
                }} 
              />
              <div className="absolute top-full left-0 mt-1.5 min-w-[200px] max-w-[280px] max-h-[250px] overflow-y-auto bg-slate-950/95 border border-cyan-500/25 rounded-xl py-1.5 text-xs text-slate-300 shadow-[0_10px_35px_rgba(0,0,0,0.8)] z-50 backdrop-blur-md animate-in fade-in slide-in-from-top-2 duration-200 divide-y divide-slate-900 border border-cyan-500/30">
                {options.map((option, index) => (
                  <button
                    ...
                    className={`w-full text-left px-3 py-2 hover:bg-cyan-500/10 hover:text-cyan-350 transition-colors flex items-center justify-between gap-2 text-slate-300 transition-all duration-155 ${
                      option.isActive ? "text-cyan-400 font-bold bg-cyan-500/5 border-l-2 border-cyan-400" : ""
                    }`}
                  >
                    <span className="truncate">{option.label}</span>
                    {option.isActive && <span className="text-[10px] text-cyan-400 shrink-0 select-none">●</span>}
                  </button>
                ))}
              </div>
            </>
      ...
      if (tooltipText) {
        return wrapWithTooltip(triggerElement, tooltipText, key, false);
      }
      return <div key={key} className="relative">{triggerElement}</div>;
    };

    // Global Top-level options list
    const homeOptions = [
      { label: "📍 Início", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("inicio"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: isHomeActive },
      { label: "📦 Produtos", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("produtos"); setTimeout(() => window.scrollTo({ top: produtosScrollY, behavior: "smooth" }), 80); }, isActive: activeTab === "produtos" },
      { label: "🔧 Ferramentas & Formatadores", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("ferramentas"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "ferramentas" },
      { label: "📱 Aplicativos (Apps)", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("apps"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "apps" },
      { label: "📝 Blog Kflow", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("blog"); setTimeout(() => window.scrollTo({ top: blogScrollY, behavior: "smooth" }), 80); }, isActive: activeTab === "blog" },
      { label: "💬 SaaS & Chatbots", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("chatbots"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "chatbots" },
      { label: "🤖 ChatKore IA", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("chatkore"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "chatkore" },
      { label: "🗓️ Kagenda", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("kagenda"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "kagenda" },
      { label: "✨ Kflow AI", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("kflow"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "kflow" },
      { label: "📊 Uptime Status", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("status"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "status" },
      { label: "🔌 Repositório de APIs", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("apis"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "apis" },
      { label: "🧮 Simulador de Projetos", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("simulador"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "simulador" },
      { label: "ℹ️ Sobre Nós", action: () => { setSelectedProduct(null); setSelectedTool(null); setSelectedApp(null); setSelectedPost(null); setIsDocPage(false); setIsPrivPage(false); setActiveTab("sobre"); window.scrollTo({ top: 0, behavior: "smooth" }); }, isActive: activeTab === "sobre" },
    ];

    // Always start with Home (clickable)
    segments.push(
      renderBreadcrumbWithDropdown(
        "home",
        <button
          onClick={() => {
            setSelectedProduct(null);
        ...
        >
          <Home className="h-3 w-3 text-blue-400 group-hover:scale-110 transition-transform shrink-0" />
          <span className="truncate max-w-[45px] xs:max-w-none">Início</span>
        </button>,
        homeOptions,
        isHomeActive,
        "Caminho: Início"
    ...
    if (isDocPage) {
      segments.push(<ChevronRight key="sep-doc" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "doc",
          <span className="text-blue-200 font-semibold px-1 flex items-center gap-1.5 min-w-0">
            <FileText className="h-3.5 w-3.5 text-blue-400 shrink-0" />
            <span className="truncate max-w-[120px] xs:max-w-[170px] sm:max-w-none">Documentação Técnica</span>
          </span>,
          [],
          true,
      ...
      segments.push(<ChevronRight key="sep-priv" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "priv",
          <span className="text-blue-200 font-semibold px-1 flex items-center gap-1.5 min-w-0">
            <ShieldCheck className="h-3.5 w-3.5 text-blue-400 shrink-0" />
            <span className="truncate max-w-[120px] xs:max-w-[170px] sm:max-w-none">Política de Privacidade</span>
          </span>,
          [],
          true,
      ...
      const prodsOptions = (db.produtos || []).map(p => ({
        label: p.nome,
        action: () => {
          setSelectedProduct(p);
          setIsDocPage(false);
      ...
      const activeProdOptions = [
        {
          label: "↩️ Ver Todos os Produtos",
          action: () => {
            setSelectedProduct(null);
            setActiveTab("produtos");
            setTimeout(() => window.scrollTo({ top: produtosScrollY, behavior: "smooth" }), 80);
          }
        },
        ...(db.produtos || []).map(p => ({
          label: p.nome,
      ...
      segments.push(<ChevronRight key="sep-prods" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "prods",
          <button
            ...
            className="text-gray-400 hover:text-blue-400 font-semibold transition cursor-pointer flex items-center gap-1.5 group min-w-0"
          >
            <Layers className="h-3.5 w-3.5 text-gray-500 group-hover:text-blue-400 transition-colors shrink-0" />
            <span className="truncate max-w-[60px] xs:max-w-[100px] sm:max-w-none">Produtos</span>
          </button>,
          prodsOptions,
          false,
      ...
      segments.push(<ChevronRight key="sep-prod" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "prod",
          <span className="text-blue-200 font-bold px-1 flex items-center gap-1.5 min-w-0">
            <Box className="h-3.5 w-3.5 text-blue-400 animate-pulse shrink-0" />
            <span className="truncate max-w-[70px] xs:max-w-[120px] sm:max-w-[180px] md:max-w-none">{selectedProduct.nome}</span>
          </span>,
          activeProdOptions,
          true,
      ...
      const toolsOptions = (db.ferramentas || []).map(f => ({
        label: f.nome,
        action: () => {
          setSelectedTool(f);
          setIsDocPage(false);
        ...
        isActive: selectedTool?.id === f.id
      }));

      const activeToolOptions = [
        {
          label: "↩️ Ver Todas as Ferramentas",
          action: () => {
            setSelectedTool(null);
            setActiveTab("ferramentas");
            window.scrollTo({ top: 0, behavior: "smooth" });
      ...
      segments.push(<ChevronRight key="sep-tools" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "tools",
          <button
            ...
            className="text-gray-400 hover:text-blue-400 font-semibold transition cursor-pointer flex items-center gap-1.5 group min-w-0"
          >
            <Wrench className="h-3.5 w-3.5 text-gray-500 group-hover:text-blue-400 transition-colors shrink-0" />
            <span className="truncate max-w-[80px] xs:max-w-[120px] sm:max-w-none">Ferramentas</span>
          </button>,
          toolsOptions,
          false,
      ...
      segments.push(<ChevronRight key="sep-tool" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "tool",
          <span className="text-blue-200 font-bold px-1 flex items-center gap-1.5 min-w-0">
            <Wrench className="h-3.5 w-3.5 text-blue-400 animate-spin shrink-0" style={{ animationDuration: '4s' }} />
            <span className="truncate max-w-[70px] xs:max-w-[120px] sm:max-w-[180px] md:max-w-none">{selectedTool.nome}</span>
          </span>,
          activeToolOptions,
          true,
      ...
      const appsOptions = (db.apps || []).map(a => ({
        label: a.nome,
        action: () => {
          setSelectedApp(a);
          setIsDocPage(false);
        ...
        isActive: selectedApp?.id === a.id
      }));

      const activeAppOptions = [
        {
          label: "↩️ Ver Todos os Aplicativos",
          action: () => {
            setSelectedApp(null);
            setActiveTab("apps");
            window.scrollTo({ top: 0, behavior: "smooth" });
      ...
      segments.push(<ChevronRight key="sep-apps" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "apps",
          <button
            ...
            className="text-gray-400 hover:text-blue-400 font-semibold transition cursor-pointer flex items-center gap-1.5 group min-w-0"
          >
            <Smartphone className="h-3.5 w-3.5 text-gray-500 group-hover:text-blue-400 transition-colors shrink-0" />
            <span className="truncate max-w-[60px] xs:max-w-[100px] sm:max-w-none">Apps</span>
          </button>,
          appsOptions,
          false,
      ...
      segments.push(<ChevronRight key="sep-app" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "app",
          <span className="text-blue-200 font-bold px-1 flex items-center gap-1.5 min-w-0">
            <Terminal className="h-3.5 w-3.5 text-blue-400 shrink-0" />
            <span className="truncate max-w-[70px] xs:max-w-[120px] sm:max-w-[180px] md:max-w-none">{selectedApp.nome}</span>
          </span>,
          activeAppOptions,
          true,
      ...
      const blogOptions = (db.blog || []).map(b => ({
        label: b.titulo,
        action: () => {
          setSelectedPost(b);
          setIsDocPage(false);
        ...
        isActive: selectedPost?.id === b.id
      }));

      const activeBlogOptions = [
        {
          label: "↩️ Ver Todos os Artigos",
          action: () => {
            setSelectedPost(null);
            setActiveTab("blog");
            setTimeout(() => window.scrollTo({ top: blogScrollY, behavior: "smooth" }), 80);
          }
        },
        ...(db.blog || []).map(b => ({
          label: b.titulo,
      ...
      segments.push(<ChevronRight key="sep-blogs" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "blogs",
          <button
            ...
            className="text-gray-400 hover:text-blue-400 font-semibold transition cursor-pointer flex items-center gap-1.5 group min-w-0"
          >
            <FileText className="h-3.5 w-3.5 text-gray-500 group-hover:text-blue-400 transition-colors shrink-0" />
            <span className="truncate max-w-[50px] xs:max-w-[90px] sm:max-w-none">Blog</span>
          </button>,
          blogOptions,
          false,
          "Caminho: Início > Blog"
      ...
      segments.push(<ChevronRight key="sep-blog" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "blog",
          <span className="text-blue-200 font-bold px-1 flex items-center gap-1.5 min-w-0">
            <Sparkles className="h-3.5 w-3.5 text-blue-400 animate-pulse shrink-0" />
            <span className="truncate max-w-[70px] xs:max-w-[120px] sm:max-w-[180px] md:max-w-none">{selectedPost.titulo}</span>
          </span>,
          activeBlogOptions,
          true,
      ...
      const tabNames: Record<string, string> = {
        ...
        kagenda: "Kagenda",
        kflow: "Kflow AI",
        ferramentas: "Ferramentas e Formatadores",
        apps: "Aplicativos",
        chatbots: "Soluções de IA & SaaS",
        chatkore: "ChatKore",
        promocoes: "Promoções",
        ...
        gadgets: "Dev Gadgets",
        "admin-dashboard": "SSO KN",
      };

      const tabIcons: Record<string, React.ReactNode> = {
        produtos: <Layers className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        kagenda: <Calendar className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        kflow: <Sparkles className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        ferramentas: <Wrench className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        apps: <Smartphone className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        chatbots: <MessageSquare className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        chatkore: <MessageSquare className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        promocoes: <Percent className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        apis: <Cpu className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        status: <Activity className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        blog: <FileText className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        sobre: <Info className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        cursos: <GraduationCap className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        gqcode: <QrCode className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        gadgets: <Sliders className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
        "admin-dashboard": <Database className="h-3.5 w-3.5 text-blue-400 shrink-0" />,
      };

      const label = tabNames[activeTab] || activeTab;
      const icon = tabIcons[activeTab] || <Sparkles className="h-3.5 w-3.5 text-blue-400 shrink-0" />;

      // In tab view without child selection, tabOptions lists other tabs
      const tabOptions = homeOptions;

      segments.push(<ChevronRight key="sep-tab" className="h-3 w-3 text-gray-600 shrink-0" />);
      segments.push(
        renderBreadcrumbWithDropdown(
          "tab",
          <span className="text-blue-200 font-bold uppercase px-1 flex items-center gap-1.5 min-w-0">
            {icon}
            <span className="truncate max-w-[80px] xs:max-w-[140px] sm:max-w-none">{label}</span>
          </span>,
          tabOptions,
          true,
    ...
    return (
      <motion.nav 
        ...
        className="flex flex-wrap items-center gap-1.5 md:gap-2 text-[10px] md:text-xs text-slate-500 font-mono mb-6 bg-slate-900/35 border border-gray-800/40 py-1.5 px-2.5 sm:px-3.5 rounded-xl w-full sm:w-fit max-w-full overflow-visible relative" 
  ...
  const renderDropdownItem = (tabId: string, label: string, desc: string, icon: React.ReactNode, customKey?: string) => {
    let isActive = activeTab === tabId && !selectedProduct && !selectedTool && !selectedApp && !selectedPost && !isDocPage && !isPrivPage;
    if (tabId === "ferramentas") {
      if (activeTab === "ferramentas") {
        if (!customKey && ferramentasSubTab === "fiscais") isActive = true;
        else if (customKey === "gadget_50" && ferramentasSubTab === "fifty") isActive = true;
        else if (customKey === "gadgets_sandbox" && ferramentasSubTab === "sandbox") isActive = true;
        else if (customKey === "gqcode" && ferramentasSubTab === "gqcode") isActive = true;
      } else {
        isActive = false;
      }
    }
    ...
    return (
      <button
        ...
        onClick={() => {
          ...
          setActiveTab("ferramentas");
          if (tabId === "ferramentas") {
            if (customKey === "gadget_50") {
              setFerramentasSubTab("fifty");
            } else if (customKey === "gadgets_sandbox") {
              setFerramentasSubTab("sandbox");
            } else if (customKey === "gqcode") {
              setFerramentasSubTab("gqcode");
            } else {
              setFerramentasSubTab("fiscais");
            }
          } else {
            setActiveTab(tabId);
          }
  ...
  const renderMobileNavItem = (tabId: string, label: string, desc: string, icon: React.ReactNode, customKey?: string) => {
    ...
    if (tabId === "ferramentas") {
      if (activeTab === "ferramentas") {
        if (!customKey && ferramentasSubTab === "fiscais") isActive = true;
        else if ((customKey === "gadget_50" || customKey === "gadget_50_mobile") && ferramentasSubTab === "fifty") isActive = true;
        else if ((customKey === "gadgets_sandbox" || customKey === "gadgets_sandbox_mobile") && ferramentasSubTab === "sandbox") isActive = true;
        else if ((customKey === "gqcode" || customKey === "gqcode_mobile") && ferramentasSubTab === "gqcode") isActive = true;
      } else {
        isActive = false;
      }
    }
    ...
    return (
      <button
        ...
        onClick={() => {
          ...
          setActiveTab("ferramentas");
          if (tabId === "ferramentas") {
            if (customKey === "gadget_50" || customKey === "gadget_50_mobile") {
              setFerramentasSubTab("fifty");
            } else if (customKey === "gadgets_sandbox" || customKey === "gadgets_sandbox_mobile") {
              setFerramentasSubTab("sandbox");
            } else if (customKey === "gqcode" || customKey === "gqcode_mobile") {
              setFerramentasSubTab("gqcode");
            } else {
              setFerramentasSubTab("fiscais");
            }
      ...
      <header className="sticky top-0 z-40 bg-[#0A0D14]/80 border-b border-gray-800 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-5">
            ...
            {renderNavButton("logistica", "Logística", <Boxes className="h-3.5 w-3.5" />)}
            {renderNavButton("simulador", "Simulador de Projetos", <Calculator className="h-3.5 w-3.5" />)}
            ...
            >
              <button 
                className={`flex items-center gap-1.5 px-2 py-1 text-[10px] uppercase tracking-wider font-semibold transition-all border-b-2 cursor-pointer ${
                  ["apps", "promocoes", "portaria", "logistica", "chatbots"].includes(activeTab) && !selectedProduct && !selectedTool && !selectedApp && !selectedPost && !isDocPage && !isPrivPage
                    ? "text-blue-400 border-blue-500 font-bold" 
                    : "text-gray-400 hover:text-blue-400 border-transparent"
                }`}
              >
              ...
              <AnimatePresence>
                {activeDropdown === "sistemas" && (
                  ...
                  >
                    ...
                    {renderDropdownItem("portaria", "Portaria & Controle", "Controle de visitantes, frotas, placas OCR e chaves corporativas", <Shield className="h-4 w-4 text-indigo-400 pointer-events-none" />)}
                    {renderDropdownItem("logistica", "WMS & Logística ERP", "Controle de estoque, expedição, CRM de vendas e faturamento de NF-e", <Boxes className="h-4 w-4 text-emerald-400 pointer-events-none" />)}
                    {renderDropdownItem("chatbots", "Soluções de IA & SaaS", "Chatbots, geração de conteúdo, automações de APIs, SaaS e agentes", <MessageSquare className="h-4 w-4 text-blue-400 pointer-events-none" />)}
            ...
            >
              <button 
                className={`flex items-center gap-1.5 px-2 py-1 text-[10px] uppercase tracking-wider font-semibold transition-all border-b-2 cursor-pointer ${
                  ["ferramentas", "apis", "status", "playground"].includes(activeTab) && !selectedProduct && !selectedTool && !selectedApp && !selectedPost && !isDocPage && !isPrivPage
                    ? "text-blue-400 border-blue-500 font-bold" 
                    : "text-gray-400 hover:text-blue-400 border-transparent"
                }`}
              >
                <span>Ferramentas e Formatadores</span>
              ...
              <AnimatePresence>
                {activeDropdown === "dev" && (
                  ...
                  >
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1 w-2.5 h-2.5 bg-slate-950 border-t border-l border-slate-800 rotate-45"></div>
                    {renderDropdownItem("ferramentas", "Formatadores & Fiscais", "Conversores, cálculos rápidos e utilitários fiscais", <Wrench className="h-4 w-4 text-amber-500 pointer-events-none" />)}
          ...
          <div className="flex items-center gap-2 md:gap-3">
            ...
            <div ref={globalSearchRef} className="relative flex items-center" id="global-navbar-search-container">
              <div className="relative flex items-center w-36 sm:w-48 md:w-64 transition-all duration-300">
                <Search className="h-3.5 w-3.5 absolute left-3 text-gray-400 shrink-0 pointer-events-none" />
                <input
                  ...
                  className="w-full pl-9 pr-8 py-1.5 bg-[#0F1420]/80 border border-gray-800 focus:border-blue-500 hover:border-gray-750 rounded-full text-xs text-white placeholder-gray-500 font-sans focus:outline-none focus:ring-1 focus:ring-blue-500/20 transition-all font-medium"
                />
                {globalSearchQuery ? (
                  <button
                    onClick={() => {
                    ...
                    className="absolute right-2.5 text-gray-500 hover:text-white p-0.5 rounded-full hover:bg-slate-800 transition"
                  >
                    <X className="h-3 w-3" />
                  </button>
                ) : (
                  <kbd className="hidden md:inline-block absolute right-2.5 px-1.5 py-0.5 bg-slate-900 border border-gray-800 rounded font-mono text-[8px] text-gray-550 uppercase select-none">
                    /
                  </kbd>
                )}
              </div>

              {/* Real-time search dropdown results overlay */}
              <AnimatePresence>
                {isGlobalDropdownOpen && globalSearchQuery.trim().length >= 2 && (
                  <motion.div
                    initial={{ opacity: 0, y: 10, scale: 0.95 }}
                    animate={{ opacity: 1, y: 0, scale: 1 }}
                    exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    transition={{ duration: 0.15 }}
                    className="absolute top-full right-0 mt-2 w-[290px] sm:w-[380px] md:w-[460px] bg-[#0E131F]/98 border border-slate-800 rounded-2xl shadow-xl p-2 z-50 filter backdrop-blur-xl max-h-[400px] overflow-y-auto scrollbar-thin"
                  >
                    {(() => {
                      const cleanQ = globalSearchQuery.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                      
                      const filteredProducts = (db.produtos || []).filter(p => 
                        p.nome.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(cleanQ) ||
                        (p.descricao || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(cleanQ)
                      ).slice(0, 3);

                      const filteredTools = (db.ferramentas || []).filter(f => 
                        f.nome.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(cleanQ) ||
                        (f.utilidade || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(cleanQ)
                      ).slice(0, 3);

                      const filteredBlog = (db.blog || []).filter(b => 
                        b.titulo.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(cleanQ) ||
                        (b.resumo || "").toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").includes(cleanQ)
                      ).slice(0, 3);

                      const totalResultsCount = filteredProducts.length + filteredTools.length + filteredBlog.length;
                        ...
                        <div className="space-y-3 p-1">
                          ...
                          {filteredProducts.length > 0 && (
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold uppercase tracking-wider text-blue-400 border-b border-blue-500/10">
                                <Layers className="h-3 w-3" />
                                <span>Produtos & Sistemas ({filteredProducts.length})</span>
                              </div>
                              <div className="space-y-0.5">
                                {filteredProducts.map((p, idx) => (
                                  <div
                                    ...
                                    className="flex items-center justify-between p-2 rounded-xl bg-slate-900/40 border border-transparent hover:border-blue-500/30 hover:bg-[#151D2F] transition-all cursor-pointer group text-left"
                                  >
                                    <div className="min-w-0 pr-2">
                                      <h4 className="text-[11px] font-semibold text-white group-hover:text-blue-400 transition-colors line-clamp-1">{p.nome}</h4>
                                      <p className="text-[9px] text-gray-400 line-clamp-1 mt-0.5">{p.descricao}</p>
                                    </div>
                                    <ArrowRight className="h-3 w-3 text-gray-650 group-hover:text-blue-400 group-hover:translate-x-0.5 transition-all shrink-0" />
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          {filteredTools.length > 0 && (
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold uppercase tracking-wider text-emerald-400 border-b border-emerald-500/10">
                                <Wrench className="h-3 w-3" />
                                <span>Ferramentas & Integrações ({filteredTools.length})</span>
                              </div>
                              <div className="space-y-0.5">
                                {filteredTools.map((t, idx) => (
                                  <div
                                    ...
                                    className="flex items-center justify-between p-2 rounded-xl bg-slate-900/40 border border-transparent hover:border-emerald-500/30 hover:bg-[#151D2F] transition-all cursor-pointer group text-left"
                                  >
                                    <div className="min-w-0 pr-2">
                                      <h4 className="text-[11px] font-semibold text-white group-hover:text-emerald-400 transition-colors line-clamp-1">{t.nome}</h4>
                                      <p className="text-[9px] text-gray-400 line-clamp-1 mt-0.5">{t.utilidade}</p>
                                    </div>
                                    <ArrowRight className="h-3 w-3 text-gray-650 group-hover:text-emerald-400 group-hover:translate-x-0.5 transition-all shrink-0" />
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          {filteredBlog.length > 0 && (
                            <div className="space-y-1">
                              <div className="flex items-center gap-1.5 px-2 py-1 text-[9px] font-bold uppercase tracking-wider text-indigo-400 border-b border-indigo-500/10">
                                <FileText className="h-3 w-3" />
                                <span>Artigos & Blog ({filteredBlog.length})</span>
                              </div>
                              <div className="space-y-0.5">
                                {filteredBlog.map((b, idx) => (
                                  <div
                                    key={`gb-${idx}`}
                                    onClick={() => {
                                      setSelectedPost(b);
                                    ...
                                    className="flex items-center justify-between p-2 rounded-xl bg-slate-900/40 border border-transparent hover:border-indigo-500/30 hover:bg-[#151D2F] transition-all cursor-pointer group text-left"
                                  >
                                    <div className="min-w-0 pr-2">
                                      <h4 className="text-[11px] font-semibold text-white group-hover:text-indigo-400 transition-colors line-clamp-1">{b.titulo}</h4>
                                      <p className="text-[9px] text-gray-400 line-clamp-1 mt-0.5">{b.resumo}</p>
                                    </div>
                                    <ArrowRight className="h-3 w-3 text-gray-650 group-hover:text-indigo-400 group-hover:translate-x-0.5 transition-all shrink-0" />
                                  </div>
                                ))}
                              </div>
                            </div>
                          ...
                          {/* Footer options */}
                          <div className="border-t border-slate-800 pt-2 px-1 flex justify-between items-center text-[9px]">
                            <button
                              onClick={() => {
                                setIsSearchOpen(true);
                                setIsGlobalDropdownOpen(false);
                              }}
                              className="text-blue-400 hover:text-blue-300 font-semibold transition cursor-pointer bg-transparent border-none p-0"
                            >
                              Ver busca completa
                            </button>
                            <span className="text-gray-500 font-mono">Real-Time Search v2</span>
                          </div>
                        </div>
                      );
                    })()}
            ...
            {isAdmin ? (
              ...
              >
                ...
                <span className="hidden sm:inline">Planilha SSO KN</span>
              </button>
            ) : (
              <button
                onClick={() => setShowAdminLogin(true)}
                className="p-2 text-slate-400 hover:text-white transition"
                title="Acesso SSO KN Oculto"
            ...
            <button
              ...
              className="p-2 text-slate-400 hover:text-white bg-slate-900/50 hover:bg-slate-900 border border-slate-800 rounded-xl transition cursor-pointer flex items-center justify-center min-h-[40px] min-w-[40px]"
              aria-label="Controle de Menu"
              id="mobile-hamburguer-toggle"
            >
              {isMobileMenuOpen ? (
                <X className="h-5 w-5 text-rose-400 transition" />
              ) : (
                <Menu className="h-5 w-5 text-slate-400 transition" />
              )}
            </button>
          </div>
        </div>

        {/* Mobile/Global Navigation Drawer Overlay */}
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              className="border-t border-gray-800 bg-[#0A0D14]/98 backdrop-blur-xl max-h-[80vh] overflow-y-auto scrollbar-none"
              id="mobile-drawer-menu"
            >
              <div className="p-4 space-y-6 md:space-y-0 md:grid md:grid-cols-3 md:gap-6 max-w-7xl mx-auto">
                {/* Category 1: Sistemas */}
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-1">
                    ...
                    {renderMobileNavItem("portaria", "Portaria & Controle", "Controle de visitantes, frotas de veículo, placas e chaves", <Shield className="h-3.8 w-3.8 text-indigo-400" />)}
                    {renderMobileNavItem("logistica", "WMS & Logística ERP", "Estoque, retiradas, NF-e, vendas e CRM", <Boxes className="h-3.8 w-3.8 text-emerald-400" />)}
                    {renderMobileNavItem("chatbots", "Soluções de IA & SaaS", "Chatbots, geração de conteúdo, automações de APIs, SaaS e agentes", <MessageSquare className="h-3.8 w-3.8 text-blue-400" />)}
                    ...
                    {renderMobileNavItem("simulador", "Simulador de Orçamentos", "Simulador inteligente de prazos, infra e custos", <Calculator className="h-3.8 w-3.8 text-blue-400" />)}
                  </div>
                </div>
                ...
                {/* Call Action Mobile */}
                <div className="pt-2 pb-1 md:col-span-3">
                  <a
                    href="https://api.whatsapp.com/send/?phone=5511989387263&text=Olá!%20Vim%20pelo%20site%20korenexus.com.br%20e%20gostaria%20de%20solicitar%20um%20orçamento%20de%20sistema."
                    target="_blank"
                    referrerPolicy="no-referrer"
      ...
      <main className="flex-1 max-w-7xl mx-auto px-4 py-8 w-full z-10">
        ...
        {loading ? (
          ...
          <>
            ...
            <PageSeoManager 
              activeTab={activeTab}
              ferramentasSubTab={ferramentasSubTab}
              selectedProduct={selectedProduct}
              selectedTool={selectedTool}
              selectedApp={selectedApp}
              selectedPost={selectedPost}
              isDocPage={isDocPage}
              isPrivPage={isPrivPage}
              onNavigate={(tab) => {
            ...
            {/* PORTARIA & SEGURANÇA CORPORATIVA VIEW */}
            {activeTab === "portaria" && (
              <motion.div
                key="portaria"
                initial={{ opacity: 0, y: 15 }}
            ...
            {/* WMS & LOGÍSTICA CORPORATIVA VIEW */}
            {activeTab === "logistica" && (
              <motion.div
                key="logistica"
                initial={{ opacity: 0, y: 15 }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6" id="products-list-grid">
                    {(() => {
                      ...
                      return filtered.map((p, index) => (
                        <div 
                          key={`${p.id || 'p'}-${index}`} 
                          className="bg-[#111622] border border-gray-800 rounded-3xl overflow-hidden hover:border-blue-500/50 transition-all flex flex-col justify-between"
                          id={`product-card-${p.id}`}
                        >
                          {/* Card cover image with lazy loading */}
                          <div className="relative w-full aspect-[16/9] overflow-hidden bg-slate-950/40 border-b border-gray-800/50">
                            <img
                              src={
                                p.id === "p1" ? "https://images.unsplash.com/photo-1551288049-bebda4e38f71?auto=format&fit=crop&w=600&q=80" : // CRM/ERP
                                p.id === "p2" ? "https://images.unsplash.com/photo-1586528116311-ad8dd3c8310d?auto=format&fit=crop&w=600&q=80" : // WMS
                                p.id === "p3" ? "https://images.unsplash.com/photo-1557597774-9d273605dfa9?auto=format&fit=crop&w=600&q=80" : // Security/Portaria
                                p.id === "p4" ? "https://images.unsplash.com/photo-1563986768609-322da13575f3?auto=format&fit=crop&w=600&q=80" : // SaaS/Plataforma
                                "https://images.unsplash.com/photo-1460925895917-afdab827c52f?auto=format&fit=crop&w=600&q=80" // general tech
                              }
                              alt={p.nome}
                              loading="lazy"
                              referrerPolicy="no-referrer"
                            ...
                            />
                            <div className="absolute inset-0 bg-gradient-to-t from-[#111622] via-transparent to-transparent opacity-80 pointer-events-none"></div>
                          </div>
            ...
            {activeTab === "ferramentas" && (
              <motion.div
                ...
                className="space-y-8 py-6"
              >
                {/* Category sub-tabs switcher at the very top of Ferramentas */}
                <div className="flex border-b border-gray-800/80 overflow-x-auto pb-px mb-6 scrollbar-none gap-2 font-sans" id="ferramentas-subtab-strip">
                  <button
                    onClick={() => setFerramentasSubTab("fiscais")}
                    className={`px-4 py-3 border-b-2 font-semibold text-xs tracking-wider uppercase transition-all duration-200 shrink-0 cursor-pointer ${
                      ferramentasSubTab === "fiscais"
                        ? "border-emerald-500 text-emerald-400 font-bold"
                        : "border-transparent text-gray-400 hover:text-white"
                    }`}
                  >
                    🛠️ Formatadores & Fiscais
                  </button>
                  <button
                    onClick={() => setFerramentasSubTab("sandbox")}
                    className={`px-4 py-3 border-b-2 font-semibold text-xs tracking-wider uppercase transition-all duration-200 shrink-0 cursor-pointer ${
                      ferramentasSubTab === "sandbox"
                        ? "border-emerald-500 text-emerald-400 font-bold"
                        : "border-transparent text-gray-400 hover:text-white"
                    }`}
                  >
                  ...
                  <button
                    onClick={() => setFerramentasSubTab("fifty")}
                    className={`px-4 py-3 border-b-2 font-semibold text-xs tracking-wider uppercase transition-all duration-200 shrink-0 cursor-pointer ${
                      ferramentasSubTab === "fifty"
                        ? "border-emerald-500 text-emerald-400 font-bold"
                        : "border-transparent text-gray-400 hover:text-white"
                    }`}
                  >
                    ⚡ 50 Super Ferramentas
                  </button>
                  <button
                    onClick={() => setFerramentasSubTab("gqcode")}
                    className={`px-4 py-3 border-b-2 font-semibold text-xs tracking-wider uppercase transition-all duration-200 shrink-0 cursor-pointer ${
                      ferramentasSubTab === "gqcode"
                        ? "border-emerald-500 text-emerald-400 font-bold"
                        : "border-transparent text-gray-400 hover:text-white"
                    }`}
                  >
                  ...
                  <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="tools-list-grid">
                      {db.ferramentas
                        ...
                        <div 
                          key={`${f.id || 'f'}-${index}`} 
                          className="bg-[#111622] border border-gray-800 rounded-3xl overflow-hidden flex flex-col justify-between hover:border-emerald-500/50 transition-all cursor-default"
                          id={`tool-card-${f.id}`}
                        >
                          {/* Card cover image with lazy loading */}
                          <div className="relative w-full aspect-[21/9] overflow-hidden bg-slate-950/40 border-b border-gray-800/40">
                            <img
                              src={
                                f.id === "f1" ? "https://images.unsplash.com/photo-1542831371-29b0f74f9713?auto=format&fit=crop&w=400&q=80" : // Code/Validation
                                f.id === "f2" ? "https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=400&q=80" : // Finance fintech
                                f.id === "f3" ? "https://images.unsplash.com/photo-1600132806370-bf17e65e942f?auto=format&fit=crop&w=400&q=80" : // Architect / devops
                                "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?auto=format&fit=crop&w=400&q=80" // laptop code
                              }
                              alt={f.nome}
                              loading="lazy"
                              referrerPolicy="no-referrer"
                            ...
                            />
                            <div className="absolute inset-0 bg-gradient-to-t from-[#111622] via-transparent to-transparent opacity-90 pointer-events-none"></div>
                          </div>
                ...
                {ferramentasSubTab === "sandbox" && (
                  <div className="space-y-4">
                    <div className="flex flex-col gap-1 pb-4 border-b border-gray-900">
                      <h2 className="text-2xl font-display font-bold text-white">Gadgets & Simuladores Sandbox</h2>
                      <p className="text-xs text-gray-400">Utilize nossos playgrounds para simular chaves NFe, decodificar tokens JWT, testar expressões regulares e validar payloads fiscais.</p>
                    </div>
                    <GadgetsPage />
                  </div>
                )}
                ...
                {ferramentasSubTab === "fifty" && (
                  <div className="space-y-4">
                    <div className="flex flex-col gap-1 pb-4 border-b border-gray-900">
                      <h2 className="text-2xl font-display font-bold text-white">⚡ 50 Super Ferramentas Avançadas</h2>
                      <p className="text-xs text-gray-400">Micro-utilitários rápidos, geradores de documentos prontos, conversores de design e utilitários de produtividade cotidiana.</p>
                    </div>
                    <FiftyMegaTools />
                  </div>
                )}
                ...
                {ferramentasSubTab === "gqcode" && (
                  <div className="space-y-4">
                    <div className="flex flex-col gap-1 pb-4 border-b border-gray-900">
                      <h2 className="text-2xl font-display font-bold text-white">📷 Gerador de QR Code & Pix Estático/Dinâmico</h2>
                      <p className="text-xs text-gray-400">Gerador integrado de códigos QR de alta densidade e códigos Pix padronizados pelo Banco Central para automação corporativa.</p>
                    </div>
                    <QrCodePage />
                  </div>
                )}
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="bg-[#111622] border border-gray-800 p-5 md:p-6 rounded-2xl space-y-4">
                      ...
                      <form 
                        ...
                        data-webmcp-tool="simulate-webhook"
                        data-webmcp-description="Simula o disparo em tempo real de um evento fiscal eletrônico formatado em JSON para o endpoint de destino do ERP."
                        data-webmcp-schema='{"webhookUrl":{"type":"string","required":true,"description":"Endereço HTTPS receptor que processará as notificações de eventos Sefaz"},"webhookEvent":{"type":"string","required":true,"description":"Chave identificadora ou tipo de evento fiscal do barramento"}}'
                      >
                        {/* URL input */}
                        <div className="space-y-1">
                          ...
                          <input
                            ...
                            data-webmcp-param="webhookUrl"
                            data-webmcp-description="Conexão HTTPS para a entrega do pacote JSON"
                          />
                        </div>
                        ...
                        <div className="space-y-1">
                          ...
                          <select
                            ...
                            className="w-full bg-[#03060c] border border-gray-800 p-2.5 rounded-lg text-white font-mono text-[11px] focus:outline-none focus:border-blue-500/50"
                            data-webmcp-param="webhookEvent"
                            data-webmcp-description="Tipo de documento logístico ou fiscal disparado"
                          >
            ...
            {activeTab === "blog" && (
              ...
              >
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 font-sans">
                  <div>
                    <h2 className="text-2xl font-display font-bold text-white font-sans">Blog Técnico & Hub de Notícias GNews</h2>
                    <p className="text-xs text-gray-400 mt-1">
                      Inovações industriais, inteligência artificial paulista e as maiores tendências mundiais de engenharia de software atualizadas em tempo real.
                    </p>
                  </div>
                  ...
                  <div className="relative w-full md:w-80">
                    ...
                    <input
                      ...
                      placeholder="Buscar notícias de tecnologia..."
                      className="w-full bg-[#111622]/80 border border-gray-800 focus:border-blue-550/50 pl-9 pr-8 py-2 text-xs rounded-xl text-white outline-none placeholder-gray-500 transition-colors font-sans focus:ring-1 focus:ring-blue-500/40"
                    />
                    {searchBlog && (
                      <button
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                      {(() => {
                        ...
                        if (filteredNews.length === 0) {
                          return (
                            <div className="col-span-3 text-center py-12 bg-[#111622] rounded-3xl border border-gray-800">
                              <p className="text-xs text-gray-400 font-mono text-center">Nenhuma notícia encontrada para "{searchBlog}".</p>
                            </div>
                          );
                        }

                        return filteredNews.map((article: any, index: number) => (
                          <article 
                            key={index}
                          ...
                          >
                            <div>
                              {article.image && (
                                <img 
                                  ...
                                  referrerPolicy="no-referrer"
                                  loading="lazy"
                                  className="w-full h-40 object-cover border-b border-gray-800/50 transition-transform duration-300 hover:scale-[1.01]" 
                                />
                              )}
                              <div className="p-6">
                                <div className="flex items-center justify-between gap-2 mb-3 font-mono">
                                  <span className="text-[10px] font-bold text-blue-400 uppercase truncate max-w-[120px]">
                                ...
                                <h3 className="text-sm font-display font-medium text-white mb-2 leading-snug line-clamp-2">
                                  {article.title}
                                </h3>
                            ...
                            <div className="p-6 pt-0 border-t border-gray-800/80 mt-4 flex items-center justify-between bg-slate-950/20 gap-2 font-sans">
                              <button
                                onClick={() => {
                                  setSelectedPost({
                                    id: `gnews-${index}`,
                ...
                <div className="bg-[#111622] p-8 rounded-3xl border border-gray-800 text-center space-y-4 max-w-2xl mx-auto">
                  ...
                  {subscribedNewsletter ? (
                    ...
                    <form 
                      ...
                      data-webmcp-tool="subscribe-newsletter"
                      data-webmcp-description="Inscreve um e-mail corporativo ou de negócios na newsletter técnica do KoreNexus para atualizações semanais."
                      data-webmcp-schema='{"email":{"type":"string","required":true,"description":"E-mail a ser registrado na newsletter de notícias de automação"}}'
                    >
                      <input
                        type="email"
                        ...
                        data-webmcp-description="E-mail profissional para fins de informativo técnico"
                      />
                      <button
                        type="submit"
          ...
          >
            ...
            >
              ...
              <form 
                ...
                className="space-y-4"
                data-webmcp-tool="submit-admin-login"
                data-webmcp-description="Autentica permissões de administrador com e-mail corporativo e senha."
                data-webmcp-schema='{"email":{"type":"string","required":true,"description":"E-mail administrativo de cadastro"},"password":{"type":"string","required":true,"description":"Senha secreta do painel de controle"}}'
              >
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    data-webmcp-param="email"
                    data-webmcp-description="E-mail corporativo de acesso admin"
                  />
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  ...
                  <input
                    ...
                    className="bg-[#0A0D14] text-white rounded-full border border-gray-800 px-4 py-2.5 text-xs text-center focus:ring-1 focus:ring-blue-500 outline-none font-mono w-full"
                    data-webmcp-param="password"
                    data-webmcp-description="Senha de acesso associada à conta administrativa"
                  />
                </div>
