...
export default function App() {
  ...
  useEffect(() => {
    const handleUrlChange = () => {
      ...
      if (segments.length === 0) {
        ...
        const validTabs = ["inicio", "produtos", "ferramentas", "apps", "chatkore", "promocoes", "apis", "status", "blog", "admin-dashboard", "sobre", "cursos", "gadgets", "playground", "sheets", "gqcode", "diagnostico"];
  ...
  const renderDropdownItem = (tabId: string, label: string, desc: string, icon: React.ReactNode) => {
    const isActive = activeTab === tabId && !selectedProduct && !selectedTool && !selectedApp && !selectedPost && !isDocPage && !isPrivPage;
    return (
      <button
        key={tabId}
  ...
  const renderMobileNavItem = (tabId: string, label: string, desc: string, icon: React.ReactNode) => {
    const isActive = activeTab === tabId && !selectedProduct && !selectedTool && !selectedApp && !selectedPost && !isDocPage && !isPrivPage;
    return (
      <button
        key={tabId}
            ...
            >
              <button 
                className={`flex items-center gap-1.5 px-2 py-1 text-[10px] uppercase tracking-wider font-semibold transition-all border-b-2 cursor-pointer ${
                  ["kflow", "kagenda", "chatkore", "apps", "promocoes", "sheets"].includes(activeTab) && !selectedProduct && !selectedTool && !selectedApp && !selectedPost && !isDocPage && !isPrivPage
                    ? "text-blue-400 border-blue-500 font-bold" 
                    : "text-gray-400 hover:text-blue-400 border-transparent"
                }`}
              >
              ...
              <AnimatePresence>
                {activeDropdown === "dev" && (
                  ...
                  >
                    ...
                    {renderDropdownItem("gadgets", "Gadgets Sandbox", "Utilitários integrados avançados e simuladores", <Terminal className="h-4 w-4 text-[#A8FF53] pointer-events-none" />)}
              ...
              <div className="p-4 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 gap-1">
                    ...
                    {renderMobileNavItem("gadgets", "Gadgets Sandbox", "Área de testes interativos", <Terminal className="h-3.8 w-3.8 text-[#A8FF53]" />)}
              ...
              <motion.div
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4" id="tools-list-grid">
                    {db.ferramentas
                      .filter((f) => {
                        if (ferramentasFilterTab === "todas") return true;
                        if (ferramentasFilterTab === "fiscais") return ["Validador API", "Fiscais"].includes(f.tipo);
                        if (ferramentasFilterTab === "fintech") return f.tipo === "Fintech";
                        if (ferramentasFilterTab === "devops") return f.tipo === "DevOps";
                        if (ferramentasFilterTab === "seguranca") return f.tipo === "Segurança";
                        return true;
                      })
                      .map((f) => (
                      <div 
