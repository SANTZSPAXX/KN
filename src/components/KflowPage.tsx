...
print(completion.choices[0].message.content)`
  ...
  return (
    <div className="space-y-12">
        ...
        <div className="relative z-10 max-w-4xl space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-4 pt-4">
            <button
              ...
              className="px-6 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-750 text-white font-bold rounded-full text-xs transition duration-250 shadow-lg shadow-indigo-600/20 flex items-center gap-1.5 cursor-pointer"
            >
              <Zap className="h-4 w-4 text-white animate-bounce" />
              <span>Usar Kflow (Console Integrado)</span>
            </button>

            {/* KoreNexus · Botão para abrir o widget em tela cheia */}
            <button
              id="korenexus-open"
              onClick={() => setIsFullScreenOverlay(true)}
              className="px-6 py-3 bg-[#0ea5e9] hover:bg-[#0aa2e0] text-white font-bold rounded-full text-xs transition duration-250 shadow-lg shadow-sky-500/20 flex items-center gap-1.5 cursor-pointer active:scale-98 select-none"
            >
              <Maximize2 className="h-4 w-4 text-white" />
              <span>Abrir KoreNexus API</span>
            </button>
        ...
        {showWidgetModal && (
          ...
          >
            <motion.div
              ...
              className="bg-[#0A0D14] border border-gray-800 rounded-3xl w-full max-w-4xl p-6 relative shadow-2xl space-y-4"
            >
              <div className="flex items-center justify-between border-b border-gray-800 pb-3">
                <div className="flex items-center gap-2">
                  <Zap className="h-4 w-4 text-indigo-400 animate-pulse" />
                  <h3 className="text-sm font-display font-medium text-white tracking-wide">Console Integrado KoreNexus (Kflow API Engine)</h3>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="p-1 px-3 bg-indigo-600/15 text-indigo-400 font-semibold border border-indigo-500/25 rounded-full text-[10px] hover:bg-indigo-600/20 transition flex items-center gap-1 cursor-pointer"
                  >
                    <span>Nova Aba</span>
              ...
              <div className="w-full max-h-[640px] rounded-xl overflow-hidden bg-slate-950/40 relative border border-gray-850">
                <KoreNexusWidget />
              </div>

              <div className="text-[10px] font-mono text-gray-500 text-center pt-1">
                Conectado com a infraestrutura neural sob medida da KoreNexus.
              </div>
            </motion.div>
          </motion.div>
        )}
      ...
      {/* Full screen overlay for KoreNexus API / Kflow widget */}
      <AnimatePresence>
        {isFullScreenOverlay && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            id="korenexus-overlay"
            className="fixed inset-0 z-[2147483647] bg-black/85 flex flex-col"
          >
            <div className="flex justify-between items-center px-4 py-2 bg-[#0b1220] border-b border-slate-800">
              <span className="text-xs font-mono font-semibold text-slate-350">KoreNexus API (Ambiente de Integração Expandido)</span>
              <button
                type="button"
                onClick={() => setIsFullScreenOverlay(false)}
                className="cursor-pointer px-3.5 py-1.5 rounded-lg border border-slate-700 bg-[#0f172a] text-slate-200 font-mono font-bold text-xs transition duration-200 hover:bg-slate-800 flex items-center gap-1 active:scale-95 select-none"
              >
            ...
            <iframe
              src="https://5c69fadb-90e6-4849-89a3-0da45120d9e1.lovableproject.com/embed"
              title="KoreNexus API"
              allow="clipboard-write"
              className="flex-1 w-full border-0 bg-[#0b1220]"
            />
          </motion.div>
        )}
      </AnimatePresence>
