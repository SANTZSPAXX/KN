...
import { Send, Bot, User, HelpCircle, PhoneCall, Compass, Check, RefreshCw, Maximize2, Minimize2 } from "lucide-react";
...
export default function ChatKore() {
  ...
  return (
    <div className={
      isFullScreen
        ? "fixed inset-0 z-[150] bg-[#0A0D14] flex flex-col w-full h-full p-4 md:p-6"
        : "w-full flex flex-col h-[calc(100vh-220px)] min-h-[600px] md:h-[calc(100vh-180px)] border border-gray-800 bg-[#0F131F] rounded-2xl overflow-hidden shadow-2xl"
    }>
      {/* Bot Header */}
      <div className={`p-4 bg-[#111622] border border-gray-800 flex items-center justify-between gap-3 ${isFullScreen ? "rounded-t-2xl border-b-0" : "border-b"}`}>
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-white text-sm font-display font-bold leading-none">
              ChatKore {isFullScreen && <span className="text-[10px] text-indigo-400 font-mono ml-2">(Modo Expandido)</span>}
            </h3>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-slate-800/80 hover:bg-slate-700 text-slate-300 font-mono font-bold rounded-lg border border-slate-700 transition cursor-pointer active:scale-95 select-none"
            title={isFullScreen ? "Sair da Tela Cheia" : "Modo Tela Cheia"}
          >
            {isFullScreen ? (
              <>
                <Minimize2 className="h-3.5 w-3.5 text-red-400" />
                <span className="hidden sm:inline">Modo Normal [Esc]</span>
              </>
            ) : (
              <>
          ...
          <a
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-full transition shrink-0"
          >
