...
export default function SeoMetricasView({ data, onRefresh, adminEmail }: SeoMetricasViewProps) {
  ...
  const [asyncFonts, setAsyncFonts] = useState(true);
  const [longCache, setLongCache] = useState(true);
  const [avoidReflow, setAvoidReflow] = useState(true);
  const [lockLayoutShift, setLockLayoutShift] = useState(true);
  const [reduceRequestChains, setReduceRequestChains] = useState(true);
  const [fixHeadingOrder, setFixHeadingOrder] = useState(true);
  const [fixColorContrast, setFixColorContrast] = useState(true);
