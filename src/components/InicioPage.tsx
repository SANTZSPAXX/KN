...
export default function InicioPage({ activeTab, setActiveTab }: InicioPageProps) {
  // Toggle profile view
  const [userType, setUserType] = useState<"visitante" | "cliente">("visitante");
  ...
  return (
    ...
    >
      {/* Dynamic Profile Toggle Bar */}
      <div className="max-w-md mx-auto bg-slate-900/60 p-1.5 rounded-full border border-slate-800/80 flex items-center justify-between shadow-2xl relative z-10 backdrop-blur-md">
        <button
          onClick={() => setUserType("visitante")}
          className={`flex-1 py-2 px-4 rounded-full text-xs font-bold transition-all duration-300 flex items-center justify-center gap-1.5 ${
            userType === "visitante"
        ...
        >
          ...
          <span>Visão Visitante</span>
        </button>
        <button
          onClick={() => setUserType("cliente")}
          className={`flex-1 py-2 px-4 rounded-full text-xs font-bold transition-all duration-300 flex items-center justify-center gap-1.5 ${
            userType === "cliente"
              ? "bg-slate-800 text-blue-400 border border-slate-705 border-slate-700/50"
              : "text-slate-400 hover:text-slate-200"
          }`}
        >
      ...
      {userType === "cliente" ? (
        ...
        >
          ...
          <div className="p-6 md:p-8 bg-gradient-to-r from-blue-950/20 to-indigo-950/20 border border-blue-500/10 rounded-3xl relative overflow-hidden">
            <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl pointer-events-none"></div>
            <div className="relative z-10 space-y-3">
              <div className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-emerald-500/10 border border-emerald-500/20 text-emerald-450 uppercase font-mono text-[9px] font-bold rounded-md animate-pulse">
                <span className="h-1.5 w-1.5 rounded-full bg-emerald-500"></span>
                Sistemas operacionais estáveis
              </div>
              <h1 className="text-2xl md:text-4xl font-black text-white tracking-tight">
                Olá, Parceiro KoreNexus!
              </h1>
              <p className="text-xs text-slate-305 text-slate-300 max-w-xl leading-relaxed">
                Bem-vindo à sua central de controle operacional de proximidade. Aqui você monitora SLAs, realiza backups instantâneos e agiliza suas customizações de software com o Diretor Técnico.
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-5 bg-slate-900/40 border border-slate-800/80 rounded-2xl space-y-2">
              <span className="text-[10px] font-mono text-slate-500 uppercase font-bold block">SLA de Atendimento</span>
              <div className="flex items-center gap-2">
                <span className="text-lg font-bold text-white">🟢 Ativo (15m med)</span>
              </div>
              <p className="text-[10px] text-slate-400 leading-normal">Garantia para Jundiaí, Itupeva e região em canais de proximidade.</p>
            </div>

            <div className="p-5 bg-slate-900/40 border border-slate-800/80 rounded-2xl space-y-2">
              <span className="text-[10px] font-mono text-slate-500 uppercase font-bold block">Autonomia Jurídica</span>
              <div className="flex items-center gap-2">
                <ShieldCheck className="h-4 w-4 text-blue-450 text-blue-450" />
                <span className="text-sm font-bold text-white">100% Código do Cliente</span>
              </div>
              <p className="text-[10px] text-slate-400 leading-normal">Repositório GitHub privado de sua propriedade, livre de restrições.</p>
            </div>

            <div className="p-5 bg-[#0a0d17] border border-blue-500/20 rounded-2xl space-y-2 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-12 h-12 bg-blue-500/10 rounded-full blur-xl"></div>
              <span className="text-[10px] font-mono text-blue-400 uppercase font-bold block">Database Próprio</span>
              <div className="flex items-center gap-1.5">
                <Database className="h-4 w-4 text-blue-400" />
                <span className="text-xs font-bold text-white">PostgreSQL & Nuvem</span>
              </div>
              <p className="text-[10px] text-slate-400 leading-normal">Sem mensalidades vitalícias por equipe ou bloqueio de faturamento.</p>
            </div>
          </div>

          {/* Core Admin integration guide */}
          <div className="bg-[#0b0e17] border border-slate-800 rounded-2xl p-6 space-y-4">
            <div className="flex items-start gap-4">
              <div className="p-3 bg-blue-500/10 border border-blue-500/25 text-blue-400 rounded-xl shrink-0">
                <Settings className="h-5 w-5" />
              </div>
              <div className="space-y-1">
                <h3 className="text-sm font-bold text-white font-sans">Acesse a Planilha Administrativa</h3>
                <p className="text-xs text-slate-400 leading-relaxed font-sans">
                  Você precisa cadastrar novas promoções, atualizar notificações integradas ou alterar as configurações de segurança? Acesse a aba superior <strong>"Painel Admin"</strong> do site de forma síncrona com sua conta para manipular os dados na hora.
                </p>
                <div className="pt-2 flex flex-wrap gap-2">
                  <button
                    onClick={() => setActiveTab("admin")}
                    className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white font-bold text-[10.5px] rounded-lg transition flex items-center gap-1.5 cursor-pointer"
                  >
                    Ir para Painel Admin
                    <ArrowRight className="h-3 w-3" />
                  </button>
                  <a
                    href="https://api.whatsapp.com/send/?phone=5511989387263&text=Olá!%20Gostaria%20de%20ajuda%20para%20configurar%20meu%20banco%20KoreNexus."
                    target="_blank"
                    className="px-3.5 py-1.5 bg-slate-900 hover:bg-slate-850 text-slate-350 border border-slate-800 hover:border-slate-700 font-bold text-[10.5px] rounded-lg transition"
                  >
          ...
          {/* Backup instructions / Security guidelines */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-5 bg-gradient-to-b from-[#090d16] to-[#060810] border border-slate-850 rounded-2xl text-left space-y-3">
              <div className="flex items-center gap-2">
                <Terminal className="h-4 w-4 text-indigo-400" />
                <h4 className="text-xs font-bold text-white font-sans">Processo de Salvamento JSON</h4>
              </div>
              <p className="text-[10.5px] text-slate-400 leading-relaxed font-sans">
                Seus dados de faturamento e canais são periodicamente guardados síncronos. Caso deseje realizar uma cópia fria local, o painel Admin oferece a opção "Exportar JSON" que baixa as chaves completas instantaneamente.
              </p>
            </div>

            <div className="p-5 bg-[#0b0e17] border border-slate-850 rounded-2xl text-left space-y-3">
              <div className="flex items-center gap-2">
                <Lock className="h-4 w-4 text-emerald-400 animate-pulse" />
                <h4 className="text-xs font-bold text-white font-sans">Padrão de Autocertificação Fina</h4>
              </div>
              <p className="text-[10.5px] text-slate-400 leading-relaxed font-sans">
                Seus dados sensíveis de provedor não trafegam em servidores de terceiros. Mantemos barramentos dedicados que impedem o vazamento de APIs fiscais da Sefaz ligada ao seu negócio.
              </p>
            </div>
          </div>

          {/* Support call-out card with live trigger */}
          <div className="p-6 bg-gradient-to-r from-[#0d1525] to-[#090e1b] border border-emerald-500/25 rounded-3xl grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
            <div className="md:col-span-8 space-y-2">
              <span className="text-[10px] uppercase font-mono font-bold text-emerald-400 block tracking-widest leading-none">CHAT DE PROXIMIDADE DIRETO</span>
              <h3 className="text-base md:text-xl font-bold text-white font-sans">Precisa de um Novo Escopo ou Ajuste de Rota?</h3>
              <p className="text-xs text-slate-350 font-sans leading-relaxed">
                Nossos engenheiros conhecem a sua base instalada. Toque no botão ao lado para iniciar um chat imediato com Yugny via WhatsApp e agendar um atendimento presencial ou call rápida de Sprints.
              </p>
            </div>
            <div className="md:col-span-4 flex justify-end w-full">
              <a
                href="https://api.whatsapp.com/send/?phone=5511989387263&text=Olá%20Yugny!%20Sou%20cliente%20KoreNexus%2520e%20gostaria%20de%20agendar%20um%20briefing%20operacional."
                target="_blank"
                className="w-full md:w-auto px-5 py-4 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs uppercase tracking-wider rounded-xl text-center shadow-lg transform hover:-translate-y-0.5 transition flex items-center justify-center gap-1.5"
              >
        ...
        >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center max-w-6xl mx-auto text-left relative">
            ...
            <div className="lg:col-span-7 space-y-6">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-gradient-to-r from-blue-500/10 to-indigo-500/10 border border-blue-500/20 rounded-full text-[10px] text-blue-400 font-bold uppercase tracking-widest leading-none w-fit">
                ...
                Engenharia Digital Premium & Software Corporativo
              </div>
              ...
              <p className="text-slate-400 text-xs md:text-sm leading-relaxed font-sans">
                Desenvolvimento profissional sob medida para indústrias, comércios e corporações de Jundiaí e região. Automatize faturamento, integre bancos legados e acabe de vez com mensalidades de software fechado.
              </p>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-3 pt-2">
                ...
                >
                  ...
                  Criar meu Plano de Ação com IA
                </a>
          ...
          <div className="max-w-4xl mx-auto space-y-6 text-left">
            <div className="text-center space-y-2">
              ...
              <h2 className="text-lg md:text-2xl font-extrabold text-white tracking-tight">Blueprints de Software Desenvolvidos pela KoreNexus</h2>
      ...
      <section id="chat-action-plan" className="max-w-4xl mx-auto bg-gradient-to-b from-[#111422]/90 to-[#0A0D18]/95 border border-slate-800 rounded-3xl p-6 md:p-8 space-y-6 relative overflow-hidden shadow-2xl">
        ...
        <div className="text-left max-w-2xl space-y-2 relative">
          <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-indigo-500/10 border border-indigo-400/20 rounded-full">
            ...
            <span className="text-[10px] font-mono font-black text-indigo-400 uppercase tracking-widest">CO-PILOTO KORENEXUS AI</span>
          </div>
          <h2 className="text-xl md:text-3xl font-extrabold text-white tracking-tight">
            Monte seu Plano de Ação em Segundos
          </h2>
          <p className="text-xs text-slate-400 leading-normal">
            Adicione as ideias do seu novo software, os maiores problemas da sua empresa ou gargalos operacionais e nossa IA construirá um roteiro de desenvolvimento sênior, com arquitetura ideal de nuvem, banco de dados e estimativa de resultados.
          </p>
        </div>
      ...
      <section className="bg-gradient-to-b from-[#090d16] to-[#05080f] rounded-3xl border border-slate-850 p-6 md:p-8 space-y-8 shadow-2xl relative overflow-hidden text-left">
        ...
        <div className="text-center max-w-2xl mx-auto space-y-2">
          ...
          <h2 className="text-xl md:text-3xl font-extrabold text-white tracking-tight">Como Transformamos seu Negócio</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
          ...
          <div className="p-6 bg-[#0E0F16] border border-red-500/10 rounded-2xl flex flex-col justify-between space-y-6 relative overflow-hidden">
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="space-y-1">
                  <span className="text-[10px] uppercase font-mono text-slate-500">Custo Financeiro Real Recorrente:</span>
                  <div className="text-2xl font-bold text-white tracking-tight animate-pulse">
                    {compareTeamSize === 2 && "R$ 32.400 / mês"}
                    {compareTeamSize === 4 && "R$ 75.600 / mês"}
                    {compareTeamSize === 8 && "R$ 153.000 / mês"}
                  </div>
                ...
                <div className="space-y-2 text-xs text-slate-400 font-sans">
                  ...
                  <p className="flex items-start gap-2">
                    <span className="text-red-400 font-bold shrink-0">✕</span>
                    <span><strong>Setup Inicial Caro</strong>: Custos de notebooks de alta performance (R$ 12.000 cada), licenças adicionais de Slack/Jira.</span>
                  </p>
                  <p className="flex items-start gap-2">
                    <span className="text-red-400 font-bold shrink-0">✕</span>
      ...
      {/* 6. Grid of Key Features */}
      <div className="space-y-6">
        <div className="text-center max-w-2xl mx-auto space-y-2">
          <span className="text-[10px] font-mono font-bold text-emerald-400 uppercase tracking-widest">NOSSAS FRENTES DE DESENVOLVIMENTO</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#111622] border border-slate-850 p-6 rounded-2xl hover:border-blue-500/50 transition-all cursor-default group flex flex-col justify-between shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-400 leading-relaxed font-sans">
                Acompanhamos o andamento operacional de faturamento sênior, emissão de notas com aprovação XML na Sefaz, estoques industriais e painéis analíticos bento.
              </p>
            </div>
          </div>

          <div className="bg-[#111622] border border-slate-850 p-6 rounded-2xl hover:border-emerald-500/50 transition-all cursor-default group flex flex-col justify-between shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-400 leading-relaxed font-sans">
                Especialistas em aplicativos offline automáticos. Sincronização inteligente para setores de faturamento externos, transporte rural ou inspeção física local.
              </p>
            </div>
          </div>

          <div className="bg-[#111622] border border-slate-850 p-6 rounded-2xl hover:border-purple-500/50 transition-all cursor-default group flex flex-col justify-between shadow-sm">
            <div>
              ...
              <p className="text-xs text-slate-400 leading-relaxed font-sans">
                Barramentos de microsserviços seguros. Integramos faturadores em lote, CRMs legados, gateways de pagamento e bots síncronos de canais digitais.
              </p>
            </div>
          </div>
        </div>
