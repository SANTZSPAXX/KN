...
export default function FaqSection({ pageId }: FaqSectionProps) {
  ...
  const toggleOpen = (id: string) => {
    // Only one key expanded at a time to keep the dashboard extremely neat and clean
    setOpenIds((prev) =>
      prev.includes(id) ? [] : [id]
    );
  };
  ...
  if (!isSectionExpanded) {
    return (
      <section 
        id="faq-section" 
        className="border border-slate-900 bg-[#06080E]/95 rounded-2xl p-5 hover:border-indigo-500/20 transition-all duration-300 cursor-pointer relative overflow-hidden group select-none"
      ...
      >
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 z-10 relative">
          ...
          <div className="flex items-start gap-4">
            <div className="p-2.5 bg-indigo-600/10 border border-indigo-500/20 rounded-xl mt-0.5 shrink-0 transition-all group-hover:scale-105 group-hover:border-indigo-500/40">
              <HelpCircle className="h-4 w-4 text-indigo-400 animate-pulse" />
            </div>
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <span className="text-[9px] font-mono font-bold text-indigo-400 uppercase tracking-widest bg-indigo-600/10 px-2 py-0.5 rounded border border-indigo-500/10">
                  FAQ Técnico
                </span>
                <span className="inline-block w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
              </div>
              <h4 className="text-sm font-semibold text-white tracking-tight group-hover:text-indigo-300 transition-colors">
                Central de Ajuda & Dúvidas Técnicas (FAQ)
              </h4>
              <p className="text-xs text-slate-400 leading-relaxed max-w-xl font-sans">
                Esclareça instantaneamente dúvidas sobre Sistemas ERP, Kflow, APIs, WhatsApp e nuvem. Possuímos mais de 190 respostas catalogadas.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 shrink-0">
            ...
            <div className="hidden lg:flex items-center gap-2 mr-2">
              <span className="text-[9px] font-mono text-slate-500 bg-[#080B12] px-2 py-0.5 rounded border border-slate-900">ERP & WMS</span>
              <span className="text-[9px] font-mono text-slate-500 bg-[#080B12] px-2 py-0.5 rounded border border-slate-900">KFLOW</span>
              <span className="text-[9px] font-mono text-slate-500 bg-[#080B12] px-2 py-0.5 rounded border border-slate-900">CHATBOT</span>
            </div>
            ...
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-xs text-white font-medium rounded-xl transition-all duration-200 cursor-pointer shadow-lg shadow-indigo-600/10 select-none font-sans scale-100 active:scale-95"
              style={{ minHeight: "38px" }}
            >
              <Search className="h-3.5 w-3.5" />
              <span>Abrir Central de FAQ</span>
      ...
      <div className="max-w-3xl mx-auto space-y-3 pt-2 z-10 relative">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-[11px] font-mono text-slate-500 px-2 pb-2.5 border-b border-slate-900">
          ...
          <div className="flex flex-wrap items-center gap-3 md:gap-4 sm:justify-end">
            ...
            <button
              ...
              className="text-[10px] text-rose-400 hover:text-rose-300 font-bold uppercase tracking-wider transition duration-200 cursor-pointer flex items-center gap-1 select-none"
            >
              [-] Minimizar FAQ
            </button>
            <span className="text-slate-800 hidden sm:inline">|</span>
        ...
        {filteredFaqs.length > 0 ? (
          <div className="space-y-3">
            ...
            {/* Elegant Minimizer footer button when list is long */}
            {filteredFaqs.length > 3 && (
              <div className="flex justify-center pt-4">
                <button
                  ...
                  className="px-4 py-2 bg-slate-900 hover:bg-rose-950/20 border border-slate-800 hover:border-rose-900/30 text-[11px] font-mono font-bold text-slate-400 hover:text-rose-300 rounded-xl transition duration-200 cursor-pointer flex items-center gap-2 select-none"
                >
                  [-] Recolher e Minimizar FAQ Completo
                </button>
              </div>
            )}
          </div>
