...
export function ToolDetailPage({ f, onBack }: ToolDetailPageProps) {
  ...
  const handleSimulateTerminal = (e: React.FormEvent) => {
    ...
    if (f.nome.toLowerCase().includes("valid")) {
      reply = `[Validation Service] Analisando payload: "${payload}" => Sefaz/RFB CNPJ verificado com sucesso. Situação cadastral: ATIVA!`;
    } else if (f.nome.toLowerCase().includes("calc")) {
      reply = `[Calc Engine] Analisando números informados: "${payload}" => Lucro Bruto estimado: 35.8% (Ok). Margem de segurança operacional ajustada.`;
    } else if (f.nome.toLowerCase().includes("sefaz") || f.nome.toLowerCase().includes("parser")) {
      reply = `[Sefaz Key Parser] Analisando chave de acesso: "${payload}"\n=> Comprimento: ${payload.length} caracteres (Ok)\n=> Estado (UF): São Paulo (SP)\n=> Emissão: 06/2026\n=> Modelo: 55 (NF-e)\n=> Série: 1\n=> Número do Documento: 001284\n=> Tipo de Emissão: 1 (Normal)\n=> Chave de Acesso Válida!`;
    } else if (f.nome.toLowerCase().includes("xml") || f.nome.toLowerCase().includes("injetor")) {
      reply = `[XML Injector Engine] Escaneando arquivo...\n=> Tag principal de notas identificada\n=> Novo bloco de tag fiscal inserido com sucesso: <impostos><difal><icms>...\n=> Status: XML re-arquivado com sucesso pronto para re-transmissão.`;
    } else if (f.nome.toLowerCase().includes("diff") || f.nome.toLowerCase().includes("db")) {
      reply = `[DB Diff Generator] Comparando estrutura em tempo de execução...\n=> Diferença identificada: Tabela "web_sessions" ausente localmente\n=> Script DDL autogerado:\n   CREATE TABLE IF NOT EXISTS web_sessions (\n     id VARCHAR(255) PRIMARY KEY,\n     mfa_status VARCHAR(50) DEFAULT 'ACTIVE'\n   );`;
    } else if (f.nome.toLowerCase().includes("sanitize") || f.nome.toLowerCase().includes("seguran")) {
      reply = `[Sanitation Guard] Inspecionando string...\n=> Caracteres especiais perigosos removidos\n=> Sem caracteres sql injection ou tags xss (<script>).\n=> String Sanitizada com Sucesso: "${payload.replace(/['";<>]/g, "")}"`;
    } else {
