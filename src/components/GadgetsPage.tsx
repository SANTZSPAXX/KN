...
import InteractiveMarketingTools from "./InteractiveMarketingTools";
import FiftyMegaTools from "./FiftyMegaTools";
import { 
  Terminal, 
  Code, 
  Shield, 
...
export default function GadgetsPage() {
  ...
  const [mainSandboxTab, setMainSandboxTab] = useState<"dev" | "business" | "fifty">("fifty");
  ...
  return (
    <div id="developer-gadgets-tab" className="space-y-8 py-4">
      ...
      <div className="flex p-1 bg-[#080b12] border border-gray-950 rounded-xl w-fit font-sans">
        <button
          onClick={() => setMainSandboxTab("fifty")}
          className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${
            mainSandboxTab === "fifty"
              ? "bg-[#10b981]/15 text-[#10b981] border border-[#10b981]/20"
              : "text-gray-400 hover:text-white"
          }`}
        >
          <span>⚡</span>
          <span>50 Super Ferramentas (Novas)</span>
        </button>
        <button
          onClick={() => setMainSandboxTab("business")}
          className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${
            mainSandboxTab === "business"
              ? "bg-blue-500/10 text-blue-400 border border-blue-500/25"
              : "text-gray-400 hover:text-white"
          }`}
        >
          <span>💼</span>
          <span>Business & SEO Suite (10)</span>
        </button>
        <button
          onClick={() => setMainSandboxTab("dev")}
          className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all cursor-pointer flex items-center gap-1.5 ${
            mainSandboxTab === "dev"
              ? "bg-[#A8FF53]/15 text-[#A8FF53] border border-[#A8FF53]/20"
              : "text-gray-400 hover:text-white"
          }`}
        >
          <span>🎛️</span>
          <span>Sefaz & DevHub SDKs</span>
        </button>
      </div>
