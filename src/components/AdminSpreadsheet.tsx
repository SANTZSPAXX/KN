...
import { 
  Save, Plus, Trash2, Download, Send, CheckCircle2, AlertCircle, Github, 
  Eye, EyeOff, Key, Settings, Database, Lock, ShieldCheck, RefreshCw, 
  Layers, Activity, Globe, Gauge, Sparkles, FileSpreadsheet, Upload,
  Pencil, Search, Filter, HelpCircle, FileCode, Check, UploadCloud, AlertTriangle,
  Copy, ArrowUpDown, ChevronUp, ChevronDown
} from "lucide-react";
...
export default function AdminSpreadsheet({ data, onUpdateData, onRefresh, adminEmail }: AdminSpreadsheetProps) {
  const [activeTab, setActiveTab] = useState<TabKey | "configuracoes" | "seo-metricas" | "seo-performance">("produtos");
  const [editingCell, setEditingCell] = useState<{ rowIndex: number; field: string } | null>(null);
  const [saveStatus, setSaveStatus] = useState<{ type: "success" | "error"; message: string } | null>(null);
  
  // Custom states to improve 100% the usability and aesthetics of the admin panel
  const [searchTerm, setSearchTerm] = useState("");
  const [statusFilter, setStatusFilter] = useState("todos");
  const [showAddForm, setShowAddForm] = useState(false);
  const [newFormValues, setNewFormValues] = useState<Record<string, string>>({});
  ...
  // Restore full backup replaces local database sheets tables and active credentials synchronously
  const handleRestoreBackup = async (file: File) => {
    ...
    try {
      ...
      reader.onload = async (e) => {
        try {
          const parsedBundle = JSON.parse(e.target?.result as string);
          if (!parsedBundle || !parsedBundle.sheetsDatabase) {
            throw new Error("Arquivo de backup inválido. Ele precisa conter as tabelas das planilhas ('sheetsDatabase').");
          }

          const response = await safeFetch("/api/admin/config/restore-all", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              email: adminEmail,
          ...
          if (result.success) {
            setRestoreStatus({
              type: "success",
              message: "🎉 Sucesso! Backup restaurado com sucesso! Clique em 'Recarregar' ou reinicie a sessão se as tabelas ainda parecerem antigas."
            });
            onRefresh();
            // Refetch config keys to sync UI
            fetchConfigKeys();
          } else {
            setRestoreStatus({
              type: "error",
              message: result.error || "Falha técnica ao tentar aplicar backup no banco."
            });
          }
        } catch (err: any) {
          setRestoreStatus({
            type: "error",
            message: "Falha de processamento de JSON: " + err.message
          });
        } finally {
      ...
      setRestoreStatus({
        type: "error",
        message: "Erro síncrono lendo arquivo: " + err.message
      });
      setRestoreLoading(false);
    }
  };
  ...
  // Whenever activeTab changes, initialize the form values with empty strings for fast add
  useEffect(() => {
    if (activeTab !== "configuracoes" && activeTab !== "seo-metricas" && activeTab !== "seo-performance") {
      const initialValues: Record<string, string> = {};
      const tabRows = data[activeTab] || [];
      const newIdCode = activeTab.substring(0, 2).toLowerCase() + (tabRows.length + 1);
      initialValues["id"] = newIdCode;
      ...
      setNewFormValues(initialValues);
    }
  }, [activeTab, data]);

  const handleConfirmAddFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const updatedRows = [...activeRows, newFormValues];
    const updatedData = {
      ...data,
    ...
    // Reset form values
    const newIdCode = activeTab.substring(0, 2).toLowerCase() + (updatedRows.length + 1);
    const initialValues: Record<string, string> = { id: newIdCode };
    const cols = tabColumns[activeTab] || [];
    cols.forEach(col => {
      if (col.key !== "id") {
        initialValues[col.key] = "";
    ...
    setSaveStatus({ type: "success", message: `Novo item adicionado com sucesso na aba '${activeTab}'! Lembre de clicar em "Salvar Alterações" para persistir.` });
    setTimeout(() => setSaveStatus(null), 5000);
  };

  const renderStatsPanel = () => {
    if (activeTab === "configuracoes" || activeTab === "seo-metricas" || activeTab === "seo-performance") return null;

    const totalCount = activeRows.length;
    let badgeText1 = "Total de Registros";
    let badgeVal1: any = totalCount;
    
    let badgeText2 = "Sistemas Ativos";
    let badgeVal2: any = 0;

    let badgeText3 = "Fase Piloto / Beta";
    let badgeVal3: any = 0;

    let bgClass = "from-blue-600/10 to-indigo-600/10 border-blue-500/20";
    let iconColor = "text-blue-400";

    const normalizedRows = activeRows || [];

    if (activeTab === "produtos") {
      badgeText2 = "Ativos em Linha";
      badgeVal2 = normalizedRows.filter((r: any) => String(r.status || "").toLowerCase().includes("ativo")).length;
      badgeText3 = "Produtos Novos";
      badgeVal3 = normalizedRows.filter((r: any) => String(r.status || "").toLowerCase().includes("novo") || String(r.status || "").toLowerCase().includes("beta")).length;
      bgClass = "from-blue-600/5 to-cyan-500/5 border-blue-500/10";
      iconColor = "text-cyan-400";
    } else if (activeTab === "ferramentas") {
      badgeText2 = "Utilitários Estáveis";
      badgeVal2 = normalizedRows.filter((r: any) => String(r.status || "").toLowerCase().includes("está") || String(r.status || "").toLowerCase().includes("esta")).length;
      badgeText3 = "Ferramentas em Beta";
      badgeVal3 = normalizedRows.filter((r: any) => String(r.status || "").toLowerCase().includes("beta") || String(r.status || "").toLowerCase().includes("novo")).length;
      bgClass = "from-emerald-600/5 to-teal-500/5 border-emerald-500/10";
      iconColor = "text-emerald-400";
    } else if (activeTab === "apps") {
      badgeText2 = "Multiplataformas";
      badgeVal2 = normalizedRows.filter((r: any) => String(r.plataforma || "").includes("/")).length;
      badgeText3 = "Downloads Estimados";
      const totalDownloadsSum = normalizedRows.reduce((acc: number, r: any) => {
        const d = parseInt(String(r.downloads || "").replace(/[^0-9]/g, ""));
        return isNaN(d) ? acc : acc + d;
      }, 0);
      badgeVal3 = totalDownloadsSum > 0 ? `${totalDownloadsSum}k+` : "48k+";
      bgClass = "from-purple-600/5 to-pink-500/5 border-purple-500/10";
      iconColor = "text-purple-400";
    } else if (activeTab === "promocoes") {
      badgeText2 = "Cupons Gratuitos";
      badgeVal2 = normalizedRows.filter((r: any) => String(r.desconto || "").includes("100%") || String(r.desconto || "").toLowerCase().includes("grát") || String(r.desconto || "").toLowerCase().includes("grat")).length;
      badgeText3 = "Ofertas Ativas";
      badgeVal3 = normalizedRows.filter((r: any) => String(r.validade || "").length > 0).length;
      bgClass = "from-amber-600/5 to-orange-500/5 border-amber-500/10";
      iconColor = "text-amber-400";
    } else if (activeTab === "blog") {
      badgeText1 = "Total de Posts";
      badgeText2 = "Assinados por Yugny";
      badgeVal2 = normalizedRows.filter((r: any) => String(r.autor || "").includes("Yugny")).length;
      badgeText3 = "Categorias Distintas";
      badgeVal3 = new Set(normalizedRows.map((r: any) => r.categoria).filter(Boolean)).size;
      bgClass = "from-indigo-600/5 to-violet-500/5 border-indigo-500/10";
      iconColor = "text-indigo-400";
    } else if (activeTab === "notificacoes") {
      badgeText1 = "Push Compartilhados";
      badgeText2 = "Remetentes Autorizados";
      badgeVal2 = new Set(normalizedRows.map((r: any) => r.enviadoPor).filter(Boolean)).size;
      badgeText3 = "Último Alerta Mensal";
      badgeVal3 = normalizedRows[0]?.titulo ? String(normalizedRows[0].titulo).substring(0, 15) + "..." : "Nenhum";
      bgClass = "from-rose-600/5 to-pink-500/5 border-rose-500/10";
      iconColor = "text-rose-400";
    }

    return (
      <div className={`grid grid-cols-1 md:grid-cols-3 gap-4 mb-6 p-4 rounded-2xl bg-gradient-to-r border ${bgClass}`}>
        <div className="flex items-center gap-3.5 p-2">
          <div className={`p-2.5 bg-[#0A0D14]/80 rounded-xl border border-gray-800 ${iconColor}`}>
            <Database className="h-4 w-4" />
          </div>
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-mono tracking-wider">{badgeText1}</span>
            <span className="text-xl font-extrabold text-white">{badgeVal1}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3.5 p-2">
          ...
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-mono tracking-wider">{badgeText2}</span>
            <span className="text-xl font-extrabold text-white">{badgeVal2}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3.5 p-2">
          ...
          <div>
            <span className="text-[10px] text-slate-400 block uppercase font-mono tracking-wider">{badgeText3}</span>
            <span className="text-xl font-extrabold text-white">{badgeVal3}</span>
          </div>
        </div>
      </div>
    );
  ...
  const tabColumns: Record<TabKey, { key: string; label: string; placeholder: string }[]> = {
    ...
    notificacoes: [
      ...
      { key: "enviadoPor", label: "Remetente SSO KN", placeholder: "admin@korenexus.com" }
    ]
  };
  ...
  // Duplicate / Clone row to speed up catalog input
  const handleCloneRow = (rowIndex: number) => {
    if (activeTab === "configuracoes" || activeTab === "seo-metricas" || activeTab === "seo-performance") return;
    const rowToClone = activeRows[rowIndex];
    if (!rowToClone) return;

    const baseId = rowToClone.id ? String(rowToClone.id).replace(/[0-9]/g, "") : activeTab.substring(0, 2);
    const newIdCode = baseId + (activeRows.length + 1);

    const clonedRow = {
      ...rowToClone,
      id: newIdCode,
      nome: rowToClone.nome ? `${rowToClone.nome} (Cópia)` : rowToClone.titulo ? `${rowToClone.titulo} (Cópia)` : `ID ${newIdCode}`
    };

    const updatedRows = [...activeRows, clonedRow];
    const updatedData = {
      ...data,
      [activeTab]: updatedRows
    };
    ...
    onUpdateData(updatedData);
    setSaveStatus({ type: "success", message: `Registro duplicado com sucesso! Lembre de clicar em "Salvar Alterações" para persistir.` });
    setTimeout(() => setSaveStatus(null), 5000);
  };

  // Toggle order / sort key
  ...
  const handleSaveSpreadsheet = async () => {
    ...
    if (activeTab === "seo-metricas" || activeTab === "seo-performance") {
      setSaveStatus({ type: "error", message: "A aba de relatórios possui caráter estritamente de exibição e leitura. Não há dados editáveis nesta aba para salvar." });
      setTimeout(() => setSaveStatus(null), 5000);
      return;
    }
  ...
  // Hilight search matches on active query string
        ...
        <>
          {parts.map((part, i) => 
            part.toLowerCase() === term.toLowerCase() ? (
              <mark key={i} className="bg-yellow-500/30 text-yellow-250 font-bold rounded-sm px-0.5">{part}</mark>
            ) : (
              part
            )
          )}
  ...
  // Format cell contents premium badges
  const renderCellContent = (key: string, value: string) => {
    const valStr = String(value || "").trim();
    if (!valStr) {
      return <span className="text-gray-650 italic font-sans text-[10px]">vazio</span>;
    }

    const content = highlightMatch(valStr, searchTerm);

    if (key === "status") {
      const lower = valStr.toLowerCase();
      if (lower.includes("ativo") || lower.includes("está") || lower.includes("esta")) {
        return (
          <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 px-2.5 py-1 rounded-full font-sans font-bold text-[10px] inline-flex items-center gap-1.5 shadow-sm">
      ...
      if (lower.includes("beta")) {
        return (
          <span className="bg-amber-500/10 text-amber-400 border border-amber-500/20 px-2.5 py-1 rounded-full font-sans font-bold text-[10px] inline-flex items-center gap-1.5 shadow-sm">
      ...
      if (lower.includes("novo")) {
        return (
          <span className="bg-blue-500/10 text-blue-400 border border-blue-500/20 px-2.5 py-1 rounded-full font-sans font-bold text-[10px] inline-flex items-center gap-1.5 shadow-sm">
      ...
      return (
        <span className="bg-slate-800/80 text-slate-300 border border-gray-700 px-2 py-0.5 rounded-full font-sans text-[10px]">
          {content}
        </span>
      );
    }
    ...
    if (key === "plataforma") {
      const containsAndroid = valStr.toLowerCase().includes("android");
      const containsIos = valStr.toLowerCase().includes("ios");
      if (containsAndroid && containsIos) {
        return (
          <span className="bg-indigo-500/10 text-indigo-400 border border-indigo-500/20 px-2 py-0.5 rounded-md font-sans text-[10px] uppercase font-bold">
            📱 {content}
          </span>
        );
      }
      if (containsAndroid) {
        return (
          <span className="bg-emerald-500/10 text-emerald-400 border border-emerald-500/25 px-2 py-0.5 rounded-md font-sans text-[10px] uppercase font-bold">
            🤖 Android
          </span>
        );
      }
      if (containsIos) {
        return (
          <span className="bg-purple-500/10 text-purple-400 border border-purple-500/25 px-2 py-0.5 rounded-md font-sans text-[10px] uppercase font-bold">
            🍎 iOS
          </span>
        );
      }
    ...
    if (key === "desconto") {
      if (valStr.includes("100%") || valStr.toLowerCase().includes("grát") || valStr.toLowerCase().includes("grat")) {
        return (
          <span className="bg-rose-500/10 text-rose-400 border border-rose-500/20 px-2.5 py-0.5 rounded-md font-sans font-extrabold text-[10px] uppercase animate-pulse">
            💥 FREE ({content})
          </span>
        );
      }
      return (
        <span className="bg-amber-500/15 text-amber-300 border border-amber-500/25 px-2 py-0.5 rounded-md font-sans font-bold text-[10px]">
          🔥 {content}
        </span>
      );
    }

    if (key === "link" && valStr.startsWith("http")) {
      return (
        <a 
          href={valStr} 
          target="_blank" 
          rel="noopener noreferrer" 
          className="text-blue-400 hover:underline inline-flex items-center gap-1 group/link font-sans"
          onClick={(e) => e.stopPropagation()}
        >
          <span className="truncate max-w-[120px]">{content}</span>
          <span className="opacity-0 group-hover/link:opacity-100 transition text-[9px]">↗</span>
        </a>
      );
    }
  ...
  return (
    <div className="w-full bg-[#111622] border border-gray-800 rounded-3xl p-6 overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-gray-800 pb-5 mb-6">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Modo Corporativo SSO KN • Conectado como <span className="text-blue-400 font-bold font-mono">{adminEmail}</span>
          </p>
        </div>
      ...
      <div className="flex flex-wrap md:flex-nowrap justify-between gap-4 bg-[#0A0D14]/80 p-1.5 rounded-2xl mb-6 border border-gray-800 items-center">
        <div className="flex flex-wrap gap-1">
          {(Object.keys(tabColumns) as TabKey[]).map((tabKey) => {
            ...
            const itemCount = data[tabKey]?.length || 0;
            return (
              <button
                key={tabKey}
                onClick={() => {
                ...
                className={`px-4 py-2 text-xs rounded-xl font-bold transition capitalize cursor-pointer flex items-center gap-2 ${
                  isActive
                ...
                id={`tab-${tabKey}`}
              >
                <span>📊 {tabKey === "promocoes" ? "Promoções" : tabKey === "notificacoes" ? "Notificações" : tabKey}</span>
                <span className={`px-1.5 py-0.5 text-[9px] rounded-full font-mono font-bold leading-none ${isActive ? "bg-white/20 text-white" : "bg-slate-800 text-slate-400"}`}>
                  {itemCount}
                </span>
              </button>
            );
        ...
        <div className="flex flex-wrap gap-2 shrink-0">
          ...
          <button
            ...
            id="tab-seo-performance"
          >
            <Globe className="h-3.5 w-3.5" />
            <span>Desempenho SEO</span>
          </button>
      ...
      {activeTab === "configuracoes" ? (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              ...
              {/* Secure JSON Backup Restore Dropzone */}
              <div className="bg-[#0A0D14]/50 border border-gray-800 p-6 rounded-3xl space-y-4">
                <div className="flex items-center gap-2 border-b border-gray-850 pb-3 text-emerald-400">
                  <UploadCloud className="h-5 w-5" />
                  <h3 className="text-sm font-semibold text-white">Restaurar do Backup</h3>
                </div>
                <p className="text-xs text-gray-400 leading-relaxed font-sans font-sans">
                  Importe um arquivo de backup JSON consolidado válido para substituir instantaneamente o banco de dados local e as credenciais operacionais em tempo de execução.
                </p>
                
                {restoreStatus && (
                  <div className={`p-3 rounded-xl border text-[11px] flex gap-2 font-sans ${
                ...
                <div 
                  className="border-2 border-dashed border-gray-800 hover:border-emerald-500/40 rounded-2xl p-6 flex flex-col items-center justify-center gap-3 transition cursor-pointer text-center bg-slate-950/25 group"
                  onClick={() => document.getElementById("file-restore-picker")?.click()}
                  onDragOver={(e) => { e.preventDefault(); e.stopPropagation(); }}
                  onDrop={(e) => {
                    e.preventDefault(); e.stopPropagation();
                    if (e.dataTransfer.files && e.dataTransfer.files[0]) {
                      handleRestoreBackup(e.dataTransfer.files[0]);
                    }
                  }}
                >
                  <input
                    id="file-restore-picker"
                    type="file"
                    accept=".json"
                    className="hidden"
                    onChange={(e) => {
                  ...
                  />
                  {restoreLoading ? (
                    <RefreshCw className="h-5 w-5 text-emerald-405 animate-spin" />
                  ) : (
                    <Upload className="h-5 w-5 text-slate-500 group-hover:text-emerald-400 transition transform group-hover:-translate-y-0.5" />
                  )}
                  <div>
                    <p className="text-[11px] font-bold text-slate-300">
                      {restoreLoading ? "Restaurando sistemas..." : "Arraste o JSON de backup aqui"}
                    </p>
        ...
        <SeoPerformanceView data={data} onRefresh={onRefresh} adminEmail={adminEmail} />
      ) : (
        <>
          {/* dynamic stats card above spreadsheet */}
          {renderStatsPanel()}

          {/* Table Helpers and Quick Form Launcher */}
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
            <div className="text-[11px] font-sans text-gray-400 flex items-center gap-1.5 leading-relaxed">
              <HelpCircle className="h-4 w-4 text-blue-400 shrink-0" />
              <span>Dica: Dê um clique em qualquer célula para editá-la diretamente, ou utilize o assistente de formulário ao lado.</span>
            </div>
            
            <button
              type="button"
              onClick={() => setShowAddForm(prev => !prev)}
              className="px-4 py-1.5 bg-blue-500/10 hover:bg-blue-500/20 border border-blue-500/20 text-blue-400 rounded-full text-[11px] font-bold transition flex items-center gap-1.5 cursor-pointer shadow-sm hover:shadow-blue-500/5 self-end md:self-auto shrink-0"
              id="btn-toggle-add-form"
            >
              <Plus className="h-3.5 w-3.5 text-blue-400" />
              <span>{showAddForm ? "Ocultar Formulário" : "Inserir via Cadastro (Rápido)"}</span>
            </button>
          </div>
          ...
          <AnimatePresence>
            {showAddForm && (
              <motion.form
                ...
                onSubmit={handleConfirmAddFormSubmit}
                className="overflow-hidden bg-[#0a0f1d]/90 border border-blue-500/10 rounded-2xl p-5 mb-5 space-y-4"
              >
                <div className="flex items-center gap-2 border-b border-gray-850 pb-2 text-blue-400">
                  <FileCode className="h-4.5 w-4.5" />
                  <h4 className="text-xs font-bold uppercase tracking-wider font-mono">Assistente de Cadastro: {activeTab}</h4>
                </div>
                  ...
                  {activeCols.map((col) => {
                    ...
                    return (
                      <div key={col.key} className="flex flex-col gap-1">
                        <label className="text-[10px] text-slate-400 font-bold uppercase tracking-wider font-mono">{col.label}</label>
                        <input
                          type="text"
                          required
                          placeholder={col.placeholder || `Insira ${col.label}`}
                          disabled={isId}
                          value={newFormValues[col.key] || ""}
                          onChange={(e) => setNewFormValues(prev => ({ ...prev, [col.key]: e.target.value }))}
                          className={`bg-[#05080E] text-xs font-sans text-slate-200 border border-gray-800 rounded-xl px-3 py-2 outline-none focus:ring-1 focus:ring-blue-500 ${isId ? "opacity-50 select-none bg-slate-900 cursor-not-allowed" : ""}`}
                        />
                      </div>
                    );
                  })}
                ...
                <div className="flex justify-end gap-2 pt-2">
                  <button
                    ...
                    className="px-4 py-2 text-xs text-slate-400 hover:text-white hover:bg-slate-800 rounded-full font-bold transition cursor-pointer"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="px-5 py-2 text-xs text-white bg-blue-600 hover:bg-blue-700 hover:shadow-lg rounded-full font-bold transition flex items-center gap-1.5 cursor-pointer"
                  >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-[#0A0D14]/90 p-4 rounded-t-2xl border border-gray-800 border-b-0">
            <div className="flex items-center gap-2 w-full sm:w-auto">
              <div className="relative w-full sm:w-72">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-slate-400" />
                <input
                  type="text"
                  placeholder={`Pesquisar na aba ${activeTab}...`}
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full bg-[#05080E] text-[#C1C9D9] text-xs border border-gray-800 rounded-full pl-9 pr-4 py-2 outline-none focus:ring-1 focus:ring-blue-500 placeholder-gray-600"
                />
              </div>
              {searchTerm && (
                <button 
                  onClick={() => setSearchTerm("")}
                  className="text-[11px] text-gray-500 hover:text-white cursor-pointer select-none font-semibold"
                >
                  Limpar
                </button>
              )}
            ...
            <div className="flex flex-wrap items-center gap-2 justify-end w-full sm:w-auto">
              ...
              <div className="flex gap-1">
                {["todos", "ativo", "beta", "novo"].map((pill) => (
                  <button
                    key={pill}
                    type="button"
                    onClick={() => setStatusFilter(pill)}
                    className={`px-3 py-1 text-[10px] font-bold rounded-full border transition cursor-pointer capitalize font-sans ${
                      statusFilter === pill
                        ? "bg-blue-600/10 text-blue-400 border-blue-500/30"
                        : "bg-transparent text-slate-400 border-gray-850 hover:text-white hover:bg-slate-800/40"
                    }`}
                  >
                    {pill}
                  </button>
          ...
          <div className="relative border border-gray-800 rounded-b-2xl overflow-hidden bg-[#0A0D14]">
            <div className="overflow-x-auto w-full max-h-[400px]">
              <table className="w-full text-left border-collapse table-auto text-xs font-mono">
                    ...
                    {activeCols.map((col) => {
                      ...
                      return (
                        <th 
                          ...
                          className="p-3 border-r border-gray-800 min-w-[150px] font-semibold uppercase tracking-wider text-gray-400 cursor-pointer hover:bg-slate-800/50 hover:text-white transition select-none group/th"
                        >
                          <div className="flex items-center justify-between gap-1.5 font-sans">
                            <span>{col.label}</span>
                            <div className="text-gray-500 group-hover/th:text-gray-300 transition shrink-0">
                              {isSorted ? (
                                isAsc ? <ChevronUp className="h-4 w-4 text-blue-400 animate-fade-in" /> : <ChevronDown className="h-4 w-4 text-blue-400 animate-fade-in" />
                              ) : (
                                <ArrowUpDown className="h-3.5 w-3.5 opacity-25 group-hover/th:opacity-100" />
                              )}
                            </div>
                          </div>
                        </th>
                ...
                <tbody>
                  {(() => {
                    // Locally filter activeRows based on search string and selected status pill
                    let filteredRows = activeRows.filter((row: any) => {
                      if (statusFilter !== "todos") {
                        const rowStatus = String(row.status || row.validade || "").toLowerCase();
                        if (statusFilter === "ativo" && !rowStatus.includes("ativo") && !rowStatus.includes("esta")) {
                          return false;
                        }
                        if (statusFilter === "novo" && !rowStatus.includes("novo")) {
                          return false;
                        }
                        if (statusFilter === "beta" && !rowStatus.includes("beta")) {
                          return false;
                        }
                      }
                    ...
                    // Sort filteredRows dynamically if sortConfig exists
                    if (sortConfig) {
                    ...
                    if (filteredRows.length === 0) {
                      return (
                        <tr>
                          <td colSpan={activeCols.length + 2} className="p-12 text-center text-gray-500 italic font-sans text-xs">
                            {searchTerm ? "Nenhum resultado corresponde à sua pesquisa." : "Aba vazia. Clique em \"Adicionar Registro\" ou \"Inserir Linha\" para iniciar."}
                          </td>
                        </tr>
                      );
                    }

                    return filteredRows.map((row: any, rowIndex) => {
                      // Map back original activeRows index to edit correctly in onCellChange
                      const originalIndex = activeRows.findIndex((r: any) => r.id === row.id);
                      const computedIndex = originalIndex !== -1 ? originalIndex : rowIndex;
                      ...
                      return (
                        <tr key={`${row.id || "row"}-${rowIndex}`} className="border-b border-gray-850 hover:bg-gray-800/10 text-slate-200 group">
                          ...
                          {activeCols.map((col) => {
                            const isEditing = editingCell?.rowIndex === computedIndex && editingCell?.field === col.key;
                            ...
                            return (
                              <td
                                key={col.key}
                                className={`p-1 border-r border-gray-800 transition-all ${
                                  isId ? "bg-slate-900/20 text-blue-400 font-bold" : "hover:bg-gray-800/30 cursor-pointer relative group/cell"
                                }`}
                                onClick={() => {
                                  if (!isId) {
                                     setEditingCell({ rowIndex: computedIndex, field: col.key });
                                  }
                                }}
                              >
                                {isEditing ? (
                                  <input
                                    type="text"
                                    value={cellValue}
                                    onChange={(e) => handleCellChange(computedIndex, col.key, e.target.value)}
                                    onBlur={() => setEditingCell(null)}
                                    placeholder={col.placeholder}
                                    autoFocus
                                    className="w-full text-xs font-sans text-white bg-[#05080E] border border-blue-500 rounded-xl px-3 py-1 outline-none"
                                    onKeyDown={(e) => {
                                        if (e.key === "Enter") {
                                          setEditingCell(null);
                                        }
                                  ...
                                  <div className="p-2 pr-6 truncate min-h-[32px] flex items-center relative group" title="Clique para editar">
                                    <span className="truncate">
                                      {renderCellContent(col.key, cellValue)}
                                    </span>
                                    {!isId && (
                                      <div className="absolute right-1.5 top-1/2 -translate-y-1/2 opacity-0 group-hover/cell:opacity-100 transition duration-150 pointer-events-none text-slate-500">
                          ...
                          <td className="p-2 bg-[#111622]/10">
                            <div className="flex items-center justify-center gap-1.5">
                              <button
                                onClick={() => handleCloneRow(computedIndex)}
                                className="p-1 text-slate-400 hover:text-white hover:bg-slate-850 rounded transition opacity-40 group-hover:opacity-100 cursor-pointer"
                                title="Duplicar item"
                                id={`btn-clone-row-${rowIndex}`}
                              >
                                <Copy className="h-3.5 w-3.5" />
                              </button>
                              
                              <button
                                onClick={() => handleDeleteRow(computedIndex)}
                                className="p-1 text-rose-500 hover:text-white hover:bg-rose-600/20 rounded transition opacity-40 group-hover:opacity-100 cursor-pointer"
                                title="Remover linha"
