...
export function getApiUrl(path: string): string {
  ...
  // Check if we are running on a known static hosting provider
  const isStaticHost = 
    hash.endsWith(".github.io") || 
    hash.endsWith(".pages.dev") || 
    hash.endsWith(".vercel.app") || 
    hash.endsWith(".netlify.app") || 
    hash.endsWith(".amplifyapp.com") ||
    hash.endsWith(".firebaseapp.com") ||
    hash.endsWith(".web.app");

  // If we are on our own custom domains, Cloud Run, localhost, or any target that isn't a third-party static-only host,
  // relative paths should be used to avoid CORS policy blocks and container auth redirects.
  const isRelativeAllowed = isCloudRun || isLocalhost || hash.includes("korenexus.com.br") || !isStaticHost;

  if (isRelativeAllowed) {
    return cleanPath;
  }
