...
if [ -f .env ]; then
    echo "🔑 Carregando configurações de ambiente seguras..."
    export $(grep -E "^GITHUB_" .env | xargs)
fi

# Ensure mandatory configurations are set
if [ -z "$GITHUB_TOKEN" ] || [ -z "$GITHUB_USER" ] || [ -z "$GITHUB_EMAIL" ]; then
    echo "❌ Erro: GITHUB_TOKEN, GITHUB_USER ou GITHUB_EMAIL não configurados no ambiente ou no .env"
    exit 1
fi
