...
const KFLOW_CACHE_FILE = path.join(process.cwd(), "data", "kflow_cache.json");
...
app.get("/api/uptime-status", async (req, res) => {
  try {
    const response = await fetch("https://api.uptimerobot.com/v2/getMonitors", {
      ...
      body: JSON.stringify({
        api_key: "ur3326395-cb7f8c8246647cc673062b76",
        format: "json",
        response_times: 1,
...
app.get("/api/news24h", async (req, res) => {
  try {
    const apikey = "4cbb11c0a29a782f763aa3179575e235";
...
const CACHE_FILE = path.join(process.cwd(), "data", "chat_cache.json");
