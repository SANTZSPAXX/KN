...
// Unified configurations from data/config.json
...
try {
  ...
  if (fs.existsSync(CONFIG_PATH)) {
    ...
    console.log("⚙️ Configurações dinâmicas carregadas com sucesso de:", CONFIG_PATH);
  } else {
    // Generate config.json if not present
    fs.writeFileSync(CONFIG_PATH, JSON.stringify(appConfig, null, 2), "utf-8");
    console.log("⚙️ Criado arquivo padrão de configurações em:", CONFIG_PATH);
  }
} catch (err) {
  console.error("⚠️ Falha ao carregar arquivo de configuração data/config.json:", err);
}

// Environment variables take precedence over config files for security & host flexibility
const GEMINI_API_KEY = process.env.GEMINI_API_KEY || appConfig.GEMINI_API_KEY;
const GNEWS_API_KEY = process.env.GNEWS_API_KEY || appConfig.GNEWS_API_KEY;
const UPTIME_ROBOT_API_KEY = process.env.UPTIME_ROBOT_API_KEY || appConfig.UPTIME_ROBOT_API_KEY;
const DB_PATH = path.isAbsolute(appConfig.DB_PATH) ? appConfig.DB_PATH : path.join(process.cwd(), appConfig.DB_PATH);
const KFLOW_CACHE_FILE = path.isAbsolute(appConfig.KFLOW_CACHE_FILE) ? appConfig.KFLOW_CACHE_FILE : path.join(process.cwd(), appConfig.KFLOW_CACHE_FILE);
const CACHE_FILE = path.isAbsolute(appConfig.CHAT_CACHE_FILE) ? appConfig.CHAT_CACHE_FILE : path.join(process.cwd(), appConfig.CHAT_CACHE_FILE);
...
function initDatabase() {
  ...
  if (!fs.existsSync(DB_PATH)) {
    ...
    // Force existing database to sign all posts under Yugny Ohany Miotelo to ensure strict consistency
    try {
      const content = fs.readFileSync(DB_PATH, "utf8");
      const dbObj = JSON.parse(content);
      if (dbObj && Array.isArray(dbObj.blog)) {
        let changed = false;
        ...
        if (changed) {
          ...
          console.log("[Seeder] Base de dados existente migrada e assinaturas de posts atualizadas.");
        }
      }
    } catch(e: any) {
